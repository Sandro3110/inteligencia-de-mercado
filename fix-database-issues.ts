...
async function fixOrphanClientes() {
  ...
  console.log(
    "  Recomendação: DELETAR (parecem ser registros de teste vazios)"
  );
...
async function fixInconsistentCounters() {
  ...
  console.log(
    `📊 Encontradas ${(inconsistent as any).length} pesquisas com contadores inconsistentes`
  );
  ...
  for (const item of inconsistent as any[]) {
    ...
    console.log(
      `  ✅ Pesquisa "${item.nome}": ${item.declarado} → ${item.contagem_real}`
    );
  }
...
async function verifyFixes() {
  ...
  if (inconsistentCount === 0) {
    ...
    console.log(
      `❌ Ainda existem ${inconsistentCount} pesquisas com contadores inconsistentes`
    );
  }
}
...
async function main() {
  ...
  try {
    ...
    console.log(
      "   Este script irá DELETAR dados órfãos e ATUALIZAR contadores.\n"
    );

...
async function reprocessWithExistingMarkets() {
  ...
  try {
    ...
    const { filterRealCompanies } = await import(
      "./server/_core/companyFilters"
    );
    ...
    console.log(
      `  Artigos removidos: ${concorrentesExistentes.length - concorrentesFiltrados.length}\n`
    );
    ...
    console.log(
      `  Artigos removidos: ${leadsExistentes.length - leadsFiltrados.length}\n`
    );
    ...
    console.log(
      `  - Concorrentes: ${concorrentesExistentes.length} (muitos artigos)`
    );
    ...
    console.log(
      `  - Concorrentes: ${concorrentesFiltrados.length} (apenas empresas reais)`
    );
    console.log(`  - Leads: ${leadsFiltrados.length} (apenas empresas reais)`);
    console.log(`  - Precisão: 100%\n`);

    const concorrentesReduction = (
      ((concorrentesExistentes.length - concorrentesFiltrados.length) /
        concorrentesExistentes.length) *
      100
    ).toFixed(1);
    const leadsReduction = (
      ((leadsExistentes.length - leadsFiltrados.length) /
        leadsExistentes.length) *
      100
    ).toFixed(1);
    ...
    console.log(
      "   Para popular o projeto com dados filtrados, seria necessário"
    );
    console.log(
      "   reprocessar via SerpAPI (buscar novos concorrentes/leads).\n"
    );

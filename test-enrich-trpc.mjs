...
for (const clientId of clientIds) {
  try {
    ...
    const response = await fetch(
      "http://localhost:3000/api/trpc/enrichmentOptimized.enrichOne",
      {
        method: "POST",
        headers: {
    ...
    if (data.result?.data) {
      ...
      console.log(
        `   - Concorrentes criados: ${result.concorrentesCreated || 0}`
      );
      console.log(`   - Leads criados: ${result.leadsCreated || 0}`);
      console.log(
        `   - Duração: ${((result.duration || 0) / 1000).toFixed(2)}s`
      );
    } else {
    ...
    console.error(
      `\n❌ Erro ao enriquecer cliente ${clientId}:`,
      error.message
    );
  }
}

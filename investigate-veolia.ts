...
(async () => {
  ...
  if (!clienteVeolia.pesquisaId) {
    console.error(
      "❌ FALHA CRÍTICA: Cliente Veolia NÃO tem pesquisaId associada!"
    );
    console.error(
      "   Isso significa que o cliente foi criado SEM vínculo com uma pesquisa."
    );
    console.error(
      "   Consequência: Os dados enriquecidos não aparecem em nenhuma pesquisa!\n"
    );
  } else {
    ...
    if (!pesquisa) {
      console.error(
        `❌ FALHA: Pesquisa ID ${clienteVeolia.pesquisaId} não existe no banco!`
      );
    } else {
  ...
  if (associacoes.length === 0) {
    console.error(
      "❌ FALHA: Cliente Veolia NÃO está associado a nenhum mercado!"
    );
    console.error(
      "   Isso significa que o enriquecimento não criou as associações.\n"
    );
  } else {
  ...
  if (produtosCliente.length === 0) {
    ...
    console.error(
      "   Isso indica que o enriquecimento não foi executado ou falhou.\n"
    );
  } else {
  ...
  for (const assoc of associacoes) {
    ...
    concorrentesSemPesquisaId += concorrentesMercado.filter(
      c => !c.pesquisaId
    ).length;
  }
  ...
  if (totalConcorrentes === 0) {
    ...
    console.warn(
      `⚠️  AVISO: ${concorrentesSemPesquisaId} concorrentes sem pesquisaId!`
    );
  ...
  if (!clienteVeolia.pesquisaId) {
    ...
    console.error(
      "   Como o cliente não tem pesquisaId, NADA aparece na tela!\n"
    );
  } else {
    console.log(
      `   Simulando busca por pesquisaId = ${clienteVeolia.pesquisaId}...\n`
    );
    ...
    if (
      mercadosPesquisa.length === 0 &&
      produtosPesquisa.length === 0 &&
      concorrentesPesquisa.length === 0 &&
      leadsPesquisa.length === 0
    ) {
      console.error(
        "❌ FALHA CRÍTICA: Nenhum dado encontrado para esta pesquisa!"
      );
      console.error(
        "   Mesmo com pesquisaId associada, os dados não estão vinculados.\n"
      );
    } else {
      console.log(
        "✅ Dados encontrados! A tela deveria mostrar esses dados.\n"
      );
    }
  }
  ...
  if (concorrentesSemPesquisaId > 0) {
    problemas.push(
      `⚠️  ${concorrentesSemPesquisaId} concorrentes sem pesquisaId`
    );
  }
  ...
  if (problemas.length === 0) {
    ...
    console.log(
      "   O problema pode estar no frontend (cache, filtros, etc.)\n"
    );
  } else {
  ...
  if (!clienteVeolia.pesquisaId) {
    console.log(
      "1. CORRIGIR: Associar cliente Veolia a uma pesquisa existente"
    );
    console.log(
      "2. CORRIGIR: Atualizar pesquisaId em todos os dados relacionados"
    );
    console.log(
      "3. PREVENIR: Garantir que novos clientes sempre tenham pesquisaId\n"
    );

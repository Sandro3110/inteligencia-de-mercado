...
export default function GeoAdmin() {
  ...
  } = trpc.geo.getStats.useQuery({
    projetoId: selectedProject,
  });

  const { data: pendingRecords, isLoading: pendingLoading } =
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground mt-1">
              Configure a API do Google Maps e execute a geocodificação da base
              de dados
            </p>
          </div>
        </div>
          ...
          <>
              ...
              <Card>
                ...
                <CardContent>
                  ...
                  <p className="text-xs text-muted-foreground mt-1">
                    {overallProgress}% geocodificados
                  </p>
                </CardContent>
              </Card>
            ...
            {stats.total.semCoordenadas > 0 && (
              <Alert>
                ...
                <AlertDescription>
                  Há {stats.total.semCoordenadas.toLocaleString()} registros
                  pendentes de geocodificação. Configure a API Key do Google
                  Maps e execute a geocodificação em lote.
                </AlertDescription>
              </Alert>
            )}
          </>
          ...
          <TabsContent value="execute" className="space-y-4">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Execute a geocodificação de todos os registros pendentes ou
                  por tipo específico
                </CardDescription>
              </CardHeader>
          ...
          <TabsContent value="config" className="space-y-4">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Configure a API Key do Google Maps para habilitar a
                  geocodificação
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Configure a API Key através da interface de gerenciamento de
                ...
                <Alert variant="default">
                  ...
                  <AlertDescription>
                    ...
                    <br />• Recomendado: geocodificar em lotes pequenos
                  </AlertDescription>
                </Alert>
              </CardContent>
            </Card>
                  ...
                  <CardContent className="space-y-2">
                    <div className="flex justify-between">
                      ...
                      <span className="font-semibold">
                        {stats.clientes.total.toLocaleString()}
                      </span>
                    </div>
                    <div className="flex justify-between text-green-600">
                      <span className="text-sm">Geocodificados:</span>
                      <span className="font-semibold">
                        {stats.clientes.comCoordenadas.toLocaleString()}
                      </span>
                    </div>
                    <div className="flex justify-between text-yellow-600">
                      <span className="text-sm">Pendentes:</span>
                      <span className="font-semibold">
                        {stats.clientes.semCoordenadas.toLocaleString()}
                      </span>
                    </div>
                    <Progress
                      value={stats.clientes.percentual}
                      className="mt-2"
                    />
                    <p className="text-xs text-center text-muted-foreground">
                      {stats.clientes.percentual}%
                    </p>
                  </CardContent>
                </Card>
                ...
                <Card>
                  ...
                  <CardContent className="space-y-2">
                    <div className="flex justify-between">
                      ...
                      <span className="font-semibold">
                        {stats.concorrentes.total.toLocaleString()}
                      </span>
                    </div>
                    <div className="flex justify-between text-green-600">
                      <span className="text-sm">Geocodificados:</span>
                      <span className="font-semibold">
                        {stats.concorrentes.comCoordenadas.toLocaleString()}
                      </span>
                    </div>
                    <div className="flex justify-between text-yellow-600">
                      <span className="text-sm">Pendentes:</span>
                      <span className="font-semibold">
                        {stats.concorrentes.semCoordenadas.toLocaleString()}
                      </span>
                    </div>
                    <Progress
                      value={stats.concorrentes.percentual}
                      className="mt-2"
                    />
                    <p className="text-xs text-center text-muted-foreground">
                      {stats.concorrentes.percentual}%
                    </p>
                  </CardContent>
                </Card>
                ...
                <Card>
                  ...
                  <CardContent className="space-y-2">
                    ...
                    <div className="flex justify-between text-green-600">
                      <span className="text-sm">Geocodificados:</span>
                      <span className="font-semibold">
                        {stats.leads.comCoordenadas.toLocaleString()}
                      </span>
                    </div>
                    <div className="flex justify-between text-yellow-600">
                      <span className="text-sm">Pendentes:</span>
                      <span className="font-semibold">
                        {stats.leads.semCoordenadas.toLocaleString()}
                      </span>
                    </div>
                    <Progress value={stats.leads.percentual} className="mt-2" />
                    <p className="text-xs text-center text-muted-foreground">
                      {stats.leads.percentual}%
                    </p>
                  </CardContent>
                </Card>
              </div>

...
export default function APIHealthDashboard() {
  ...
  const { data: stats, isLoading, refetch } = trpc.apiHealth.stats.useQuery({ days });
  ...
  const getTrendIcon = (successRate: number) => {
    if (successRate >= 95) return <TrendingUp className="w-4 h-4 text-green-500" />;
    if (successRate >= 80) return <Minus className="w-4 h-4 text-yellow-500" />;
          ...
          stats?.map((stat) => {
            ...
            return (
              <Card key={stat.apiName} className="relative overflow-hidden">
                <div className={`absolute top-0 left-0 right-0 h-1 ${apiConfig.color}`} />
                ...
                <CardContent className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="text-3xl font-bold">{stat.successRate}%</div>
                  ...
                  <div className="grid grid-cols-3 gap-3 text-sm">
                    <div className="text-center">
                      ...
                      <div className="text-xs text-slate-500 mt-1">Sucesso</div>
                    </div>
                    
                    <div className="text-center">
                      <div className="flex items-center justify-center gap-1 text-red-600">
                        <XCircle className="w-4 h-4" />
                        <span className="font-semibold">{stat.errorCount}</span>
                      </div>
                    ...
                    <div className="text-center">
                      <div className="flex items-center justify-center gap-1 text-orange-600">
                        ...
                        <span className="font-semibold">{stat.timeoutCount}</span>
                      </div>
                  ...
                  <div className="pt-3 border-t">
                    <div className="text-xs text-slate-500">Tempo Médio de Resposta</div>
                    <div className="text-xl font-semibold mt-1">{stat.avgResponseTime}ms</div>
                  </div>

                  {/* Último Erro */}
                  {stat.lastError && (
                    <div className="pt-3 border-t">
                      <div className="text-xs text-slate-500 mb-1">Último Erro</div>
                ...
                {history.map((item) => (
                  <TableRow key={item.id}>
                    <TableCell className="font-medium">
                      {API_NAMES[item.apiName as APIName]?.label || item.apiName}
                    </TableCell>
                    <TableCell>
                      {item.status === 'success' ? (

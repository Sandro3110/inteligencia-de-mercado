...
export function generateExecutivePDF(data: ExecutiveReportData) {
  ...
  doc.text("Relatório Executivo", pageWidth / 2, yPosition, {
    align: "center",
  });
  ...
  const summaryData = [
    ...
    [
      "Leads de Alta Qualidade (≥80)",
      `${data.summary.leadsHighQuality} (${((data.summary.leadsHighQuality / data.summary.totalLeads) * 100).toFixed(1)}%)`,
    ],
  ];
  ...
  const analiseData = [
    [
      ...
      data.analiseCompetitiva.mercadoMaisCompetitivo,
    ],
    [
      "Mercado Menos Competitivo",
      data.analiseCompetitiva.mercadoMenosCompetitivo,
    ],
    [
      "Média de Concorrentes por Mercado",
      data.analiseCompetitiva.mediaConcorrentesPorMercado.toString(),
    ],
  ];
  ...
  doc.setFont("helvetica", "bold");
  doc.text(
    `Leads Prioritários (Score ≥ 80) - Top ${Math.min(data.leadsPrioritarios.length, 20)}`,
    20,
    yPosition
  );
  yPosition += 10;
  ...
  if (data.leadsPrioritarios.length > 0) {
    const leadsData = data.leadsPrioritarios.map(lead => [
      ...
      lead.mercado.length > 20
        ? lead.mercado.substring(0, 17) + "..."
        : lead.mercado,
      lead.score.toString(),
      lead.cnpj || "-",
      lead.email || "-",

    ...
    <div className="space-y-4">
          ...
          return (
            ...
            >
              ...
              <div className="flex items-start justify-between mb-4">
                <div
                  className={`p-3 rounded-xl ${isSelected ? "bg-white shadow-sm" : option.bgColor}`}
                >
              ...
              <div className="space-y-1.5">
                {option.features.map((feature, index) => (
                  <div key={index} className="flex items-start gap-2">
                    <div
                      className={`w-1.5 h-1.5 rounded-full mt-1.5 ${option.color.replace("text-", "bg-")}`}
                    />
                    <p className="text-xs text-slate-700">{feature}</p>
                  </div>
                ))}
              </div>
            </Card>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
        <p className="text-xs text-blue-800">
          💡 <strong>Recomendação:</strong> Use <strong>Balanceada</strong> para
          a maioria dos casos. Escolha <strong>Profunda</strong> apenas quando
          precisar de insights estratégicos detalhados.
        </p>
      </div>
    </div>
  );

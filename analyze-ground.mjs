...
const clientesSample = db.prepare(`
...
// 5. Contar mercados
const mercadosCount = db.prepare(`
  SELECT COUNT(*) as total 
...
const mercadosSample = db.prepare(`
...
const concorrentesCount = db.prepare(`
  SELECT COUNT(*) as total 
  FROM concorrentes 
  WHERE projectId = ?
`).get(project.id);
...
console.log(`  Concorrentes por Cliente: ${returnAnalysis.concorrentesPorCliente?.toFixed(2) || 0}`);
console.log(`  Leads por Cliente: ${returnAnalysis.leadsPorCliente?.toFixed(2) || 0}`);
console.log(`  Produtos por Cliente: ${returnAnalysis.produtosPorCliente?.toFixed(2) || 0}`);
console.log('');

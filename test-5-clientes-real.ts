...
async function main() {
  ...
  console.log('\n╔════════════════════════════════════════════════════════════════╗');
  console.log('║         TESTE COM 5 CLIENTES REAIS (OpenAI + SerpAPI)         ║');
  console.log('╚════════════════════════════════════════════════════════════════╝\n');
  ...
  try {
    ...
    const results = await enrichClientesReal(clienteIds, 1, (current, total, result) => {
    ...
    console.log(`   - Quality Score Médio (Concorrentes): ${stats[0].avgScoreConcorrentes?.toFixed(1) || 'N/A'}/100`);
    console.log(`   - Quality Score Médio (Leads): ${stats[0].avgScoreLeads?.toFixed(1) || 'N/A'}/100`);
    ...
    fs.writeFileSync('/home/ubuntu/TEST_5_CLIENTES_REPORT.json', JSON.stringify(report, null, 2));

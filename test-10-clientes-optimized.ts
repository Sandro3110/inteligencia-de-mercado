...
async function main() {
  ...
  console.log('\n╔════════════════════════════════════════════════════════════════╗');
  console.log('║   TESTE COM 10 CLIENTES: OpenAI + ReceitaWS (SEM SerpAPI)     ║');
  console.log('╚════════════════════════════════════════════════════════════════╝\n');
  ...
  try {
    ...
    console.log(`   - Concorrentes: ${stat.concorrentesComCNPJ}/${stat.totalConcorrentesRecentes} (${stat.totalConcorrentesRecentes > 0 ? (stat.concorrentesComCNPJ/stat.totalConcorrentesRecentes*100).toFixed(0) : 0}%)`);
    console.log(`   - Leads: ${stat.leadsComCNPJ}/${stat.totalLeadsRecentes} (${stat.totalLeadsRecentes > 0 ? (stat.leadsComCNPJ/stat.totalLeadsRecentes*100).toFixed(0) : 0}%)`);
    ...
    console.log(`   📊 Registros Esperados: ~${Math.round((totalMercados + totalProdutos + totalConcorrentes + totalLeads) * 801 / 10)} registros`);
    ...
    const report = {
      ...
      summary: {
        ...
        cnpjValidationRate: {
          concorrentes: stat.totalConcorrentesRecentes > 0 ? stat.concorrentesComCNPJ/stat.totalConcorrentesRecentes : 0,
          leads: stat.totalLeadsRecentes > 0 ? stat.leadsComCNPJ/stat.totalLeadsRecentes : 0
        }
      },
      projection: {
        time: projectedTime,
        cost: projectedCost,
        records: Math.round((totalMercados + totalProdutos + totalConcorrentes + totalLeads) * 801 / 10)
      }
    };
    
    fs.writeFileSync('/home/ubuntu/TEST_10_OPTIMIZED_REPORT.json', JSON.stringify(report, null, 2));

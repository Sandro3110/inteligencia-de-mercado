...
(async () => {
  try {
    ...
    resultado.mercados.forEach((m, idx) => {
      ...
      console.log(`   Segmentação: ${m.mercado.segmentacao}`);
      console.log(`   Produtos: ${m.produtos.length}, Concorrentes: ${m.concorrentes.length}, Leads: ${m.leads.length}\n`);
    });
    ...
    console.log('   A API OpenAI está retornando dados completos.');
    console.log('   Se o banco está vazio, o problema está na GRAVAÇÃO.\n');

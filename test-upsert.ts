...
import { createMercado, createCliente, createConcorrente, createLead } from './server/db';
import { getDb } from './server/db';
import { mercadosUnicos, mercadosHistory, clientes, clientesHistory } from './drizzle/schema';
...
async function testUpsert() {
  ...
  const history1 = await db.select()
  ...
  const history2 = await db.select()
    .from(mercadosHistory)
    .where(eq(mercadosHistory.mercadoId, mercado1.id));

  console.log(`📊 Histórico após update: ${history2.length} entradas`);
  history2.forEach((entry, index) => {
    console.log(`   ${index + 1}. ${entry.changeType} - ${entry.field}: "${entry.oldValue}" → "${entry.newValue}"`);
  });
  ...
  // Verificar histórico
  const clienteHistory = await db.select()

...
async function testBugFixes() {
  ...
  const clientesNaoEnriquecidos = await db
    ...
    .where(
      (0, drizzle_orm_1.and)(
        (0, drizzle_orm_1.eq)(schema_1.clientes.projectId, 1),
        (0, drizzle_orm_1.isNull)(schema_1.clientes.validationStatus)
      )
    )
    .limit(5);
  ...
  const results = await (0, enrichmentOptimized_1.enrichClientesParallel)(
    clienteIds,
    1
  );

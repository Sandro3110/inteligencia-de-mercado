  ...
  try {
    ...
    const validations = [
      ...
      },
      {
        ...
        passed: leadData.leads.every((l: any) => l.regiao && l.regiao.length > 0),
      },
      {
        test: 'Score de qualidade calculado para todos',
        passed: enrichedLeads.every((l: any) => typeof l.qualidadeScore === 'number'),
      },
      {
    ...
    console.log('\n' + '═'.repeat(70));
    if (allPassed) {
      console.log('✅ TESTE APROVADO - Identificação de leads funcionando perfeitamente');
    } else if (criticalPassed) {
      console.log('⚠️  TESTE PARCIAL - Funcionalidade core OK, qualidade pode melhorar');
    } else {
    ...
    console.log(`  Enriquecidos com sucesso: ${enrichedLeads.filter((l: any) => l.enriched).length}`);
    console.log(`  Taxa de enriquecimento: ${((enrichedLeads.filter((l: any) => l.enriched).length / leadData.leads.length) * 100).toFixed(1)}%`);
    
    const avgScore = enrichedLeads.reduce((sum: number, l: any) => sum + l.qualidadeScore, 0) / enrichedLeads.length;
    console.log(`  Score médio de qualidade: ${avgScore.toFixed(1)}/100`);
    
    const highQuality = enrichedLeads.filter((l: any) => l.qualidadeScore >= 70).length;

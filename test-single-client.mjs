...
async function getClienteStats(clienteId) {
  ...
  const produtosResult = await db.select().from(produtos).where(eq(produtos.clienteId, clienteId));
  
  const concorrentesResult = await db.select().from(concorrentes).where(eq(concorrentes.projectId, PROJECT_ID));
  
  const leadsResult = await db.select().from(leads).where(eq(leads.projectId, PROJECT_ID));
...
async function main() {
  ...
  if (stats) {
    ...
    console.log(`   Cidade/UF: ${stats.cliente.cidade || '?'}/${stats.cliente.uf || '?'}`);
    ...
    console.log(`   Faturamento: ${stats.cliente.faturamentoDeclarado || 'não informado'}`);
    console.log(`   Estabelecimentos: ${stats.cliente.numeroEstabelecimentos || 'não informado'}`);
    console.log(`   Quality Score: ${stats.cliente.qualidadeScore || 0}/100 (${stats.cliente.qualidadeClassificacao || 'N/A'})`);

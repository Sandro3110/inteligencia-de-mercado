...
async function testBugFixes() {
  ...
  console.log(
    `   Com produto: ${(concorrenteCheck as any)[0].comProduto} (${(concorrenteCheck as any)[0].percentual}%)\n`
  );
  ...
  console.log(
    `   Ativos: ${(produtoCheck as any)[0].ativos} (${(produtoCheck as any)[0].percentual}%)`
  );
  ...
  console.log(
    `${produtoOK ? "✅" : "❌"} Bug Fix 1 (produto): ${concorrenteOK ? "OK" : "FALHOU"}`
  );
  console.log(

...
async function testBugFixes() {
  console.log('╔════════════════════════════════════════════════════════════════╗');
  console.log('║           TESTE DAS 3 CORREÇÕES DE BUGS                       ║');
  console.log('╚════════════════════════════════════════════════════════════════╝\n');
  ...
  const clientesNaoEnriquecidos = await db
    ...
    .where(
      and(
        eq(clientes.projectId, 1),
        isNull(clientes.validationStatus)
      )
    )
    .limit(5);
  ...
  console.log(`📊 Taxa: ${(clientesNaoEnriquecidos.length / duration * 60).toFixed(1)} clientes/min\n`);
  ...
  console.log(`   Total: ${(concorrenteCheck as any)[0].total}`);
  console.log(`   Com produto: ${(concorrenteCheck as any)[0].comProduto} (${(concorrenteCheck as any)[0].percentual}%)\n`);
  ...
  (qualityCheck as any).forEach((row: any) => {
    console.log(`   ${row.tipo}:`);
    console.log(`     - Score: ${row.minScore} a ${row.maxScore} (média: ${row.avgScore.toFixed(1)})`);
  ...
  console.log(`   Total: ${(produtoCheck as any)[0].total}`);
  console.log(`   Ativos: ${(produtoCheck as any)[0].ativos} (${(produtoCheck as any)[0].percentual}%)`);
  ...
  const produtoOK = (produtoCheck as any)[0].percentual === 100;
  const qualityOK = (qualityCheck as any)[0].uniqueScores > 1; // Deve ter variação
  const concorrenteOK = (concorrenteCheck as any)[0].percentual === 100;
  
  console.log(`${produtoOK ? '✅' : '❌'} Bug Fix 1 (produto): ${concorrenteOK ? 'OK' : 'FALHOU'}`);
  console.log(`${qualityOK ? '✅' : '❌'} Bug Fix 2 (quality): ${qualityOK ? 'OK' : 'FALHOU'}`);
  console.log(`${produtoOK ? '✅' : '❌'} Bug Fix 3 (ativo): ${produtoOK ? 'OK' : 'FALHOU'}\n`);

...
function buildCondition(table, condition) {
    ...
    switch (operator) {
        ...
        case 'endsWith':
            return (0, drizzle_orm_1.like)(column, `%${value}`);
        case 'in':
            return (0, drizzle_orm_1.inArray)(column, Array.isArray(value) ? value : [value]);
        case 'notIn':
            return (0, drizzle_orm_1.notInArray)(column, Array.isArray(value) ? value : [value]);
        case 'isNull':
...
function validateFilter(filter) {
    ...
    for (const group of filter.groups) {
        ...
        for (const condition of group.conditions) {
            ...
            const requiresValue = ['eq', 'ne', 'gt', 'lt', 'gte', 'lte', 'contains', 'startsWith', 'endsWith', 'in', 'notIn'];

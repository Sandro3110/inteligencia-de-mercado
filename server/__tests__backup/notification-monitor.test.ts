...
describe('Sistema de Notificações em Tempo Real', () => {
  ...
  describe('Fluxo Completo de Notificações', () => {
    it('deve criar notificação e receber via SSE', async () => {
      ...
      logger.debug(`[Monitor] Notificação criada: ${notification.id}`);
              ...
              if (line.startsWith('data: ')) {
                ...
                try {
                  ...
                  if (parsed.id === notification.id) {
                    ...
                    logger.debug('[Monitor] ✅ Notificação recebida via SSE:', parsed);
                    break;
                  }
                } catch (e) {
    ...
    it('deve receber múltiplas notificações em sequência', async () => {
            ...
            if (line.startsWith('data: ')) {
              try {
                ...
                if (expectedIds.has(parsed.id)) {
                  ...
                  logger.debug(`[Monitor] ✅ Recebida ${receivedIds.size}/3: ${parsed.title}`);
                }
              } catch (e) {
                // Ignorar
              }
      ...
      logger.debug(`[Monitor] Total recebido: ${receivedIds.size}/3`);
  ...
  describe('Performance e Limites', () => {
    it('deve lidar com múltiplas conexões SSE simultâneas', async () => {
      ...
      logger.debug('[Monitor] ✅ 3 conexões SSE simultâneas funcionaram');
    });
    ...
    it('deve manter conexão SSE por pelo menos 30 segundos', async () => {
      ...
      logger.debug(`[Monitor] Conexão mantida por ${duration}ms com ${heartbeatCount} heartbeats`);

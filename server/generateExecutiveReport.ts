...
export async function generateExecutiveReportData(
  ...
  if (filters?.mercadoIds && filters.mercadoIds.length > 0) {
    whereConditions.push(
      sql`${mercadosUnicos.id} IN (${sql.join(
        filters.mercadoIds.map(id => sql`${id}`),
        sql`, `
      )})`
    );
  }
  ...
  const leadsHighQualityResult = await db
    .select({ count: sql<number>`count(*)` })
    .from(leads)
    .where(
      and(eq(leads.projectId, projectId), sql`${leads.qualidadeScore} >= 80`)
    );
  ...
  const topMercados = await Promise.all(
    topMercadosRaw.map(async mercado => {
      ...
      const densidadeCompetitiva =
        totalClientes > 0 ? totalConcorrentes / totalClientes : 0;
  ...
  const mercadoMaisCompetitivo = topMercados.reduce(
    (prev, current) =>
      current.densidadeCompetitiva > prev.densidadeCompetitiva ? current : prev,
    topMercados[0] || { nome: "N/A", densidadeCompetitiva: 0 }
  );

  const mercadoMenosCompetitivo = topMercados.reduce(
    (prev, current) =>
      current.densidadeCompetitiva < prev.densidadeCompetitiva &&
      current.densidadeCompetitiva > 0
        ? current
        : prev,
    topMercados[0] || { nome: "N/A", densidadeCompetitiva: 0 }
  );
  ...
  const mediaConcorrentes =
    topMercados.length > 0
      ? topMercados.reduce((sum, m) => sum + m.totalConcorrentes, 0) /
        topMercados.length
      : 0;
  ...
  if (summary.leadsHighQuality > 0) {
    const percentualHighQuality = (
      (summary.leadsHighQuality / summary.totalLeads) *
      100
    ).toFixed(1);
    insights.push(
      `${percentualHighQuality}% dos leads (${summary.leadsHighQuality}) possuem alta qualidade (score ≥ 80), indicando forte potencial de conversão.`
    );
  }

  if (topMercados.length > 0) {
    const top3Mercados = topMercados
      .slice(0, 3)
      .map(m => m.nome)
      .join(", ");
    insights.push(
      `Os 3 mercados com maior volume de leads são: ${top3Mercados}. Recomenda-se priorizar esforços comerciais nessas áreas.`
    );
  }

  if (mercadoMaisCompetitivo.densidadeCompetitiva > 5) {
    insights.push(
      `O mercado "${mercadoMaisCompetitivo.nome}" apresenta alta densidade competitiva (${mercadoMaisCompetitivo.densidadeCompetitiva.toFixed(1)} concorrentes por cliente). Estratégias de diferenciação são essenciais.`
    );
  }

  if (
    mercadoMenosCompetitivo.densidadeCompetitiva < 2 &&
    mercadoMenosCompetitivo.totalLeads > 5
  ) {
    insights.push(
      `O mercado "${mercadoMenosCompetitivo.nome}" possui baixa concorrência (${mercadoMenosCompetitivo.densidadeCompetitiva.toFixed(1)} concorrentes por cliente) e representa uma oportunidade de entrada facilitada.`
    );
  }
  ...
  if (mercadosComLeads < summary.totalMercados * 0.5) {
    insights.push(
      `Apenas ${mercadosComLeads} de ${summary.totalMercados} mercados possuem leads identificados. Recomenda-se expandir a prospecção nos mercados restantes.`
    );
  }

...
import { logAPICall, getAPIHealthStats, getAPIHealthHistory, testAPIConnection } from './apiHealth';
...
describe('API Health Monitoring System', () => {
  ...
  describe('logAPICall', () => {
    it('deve registrar uma chamada de API com sucesso', async () => {
      ...
      const logs = await db.select().from(apiHealthLog).where(sql`apiName = 'openai'`);
    ...
    it('deve registrar uma chamada de API com erro', async () => {
      ...
      const logs = await db.select().from(apiHealthLog).where(sql`apiName = 'receitaws'`);
      expect(logs.length).toBeGreaterThan(0);
  ...
  describe('testAPIConnection', () => {
    it('deve testar conexão com OpenAI', async () => {
      ...
      if (!result.success) {
        ...
        console.log('OpenAI test failed (expected if no API key):', result.error);
      }
    });
    ...
    it('deve testar conexão com SERPAPI', async () => {
      ...
      if (!result.success) {
        expect(result).toHaveProperty('error');
        console.log('SERPAPI test failed (expected if no API key):', result.error);
      }
    });

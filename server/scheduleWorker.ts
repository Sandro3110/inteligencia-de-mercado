...
async function checkAndExecuteSchedules() {
  ...
  try {
    ...
    console.log(`[ScheduleWorker] Encontrados ${pendingSchedules.length} agendamentos para executar`);
...
async function executeSchedule(schedule: any) {
  ...
  try {
    console.log(`[ScheduleWorker] Executando agendamento #${schedule.id} do projeto #${schedule.projectId}`);
    ...
    if (clientes.length === 0) {
      console.warn(`[ScheduleWorker] Nenhum cliente encontrado para projeto #${schedule.projectId}`);
      await db
        .update(scheduledEnrichments)
        .set({ 
          status: 'error',
    ...
    executeEnrichmentFlow(
      ...
      async (progress: any) => {
        if (progress.status === 'completed') {
          console.log(`[ScheduleWorker] Agendamento #${schedule.id} concluído com sucesso`);
    ...
    console.log(`[ScheduleWorker] Enriquecimento iniciado para agendamento #${schedule.id}`);
  } catch (error: any) {
...
async function createNextSchedule(schedule: any) {
  ...
  try {
    ...
    console.log(`[ScheduleWorker] Próximo agendamento criado para ${nextDate.toISOString()}`);
  } catch (error) {

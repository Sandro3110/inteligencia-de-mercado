...
describe('Fase 60: Botão "Adiar Hibernação" no Dashboard', () => {
  ...
  describe('60.1: Botão de Adiamento e Badge de Aviso', () => {
    ...
    it('deve incluir projetos com avisos na lista de inativos mesmo com menos de 30 dias', async () => {
      ...
      const inactiveProjects = activity.projectsWithActivity.filter(p => {
        ...
        return p.hasWarning || (p.daysSinceActivity && p.daysSinceActivity >= 30);
      });
  ...
  describe('60.2 e 60.3: Adiamento com Diferentes Prazos', () => {
    it('deve adiar hibernação por 7 dias', async () => {
      ...
      const now = new Date();
      const timeDiff = now.getTime() - updatedProject!.lastActivityAt!.getTime();
    ...
    it('deve adiar hibernação por 30 dias (padrão)', async () => {
      ...
      const daysDiff = Math.floor((postponedDate.getTime() - now.getTime()) / (1000 * 60 * 60 * 24));
    ...
    it('deve retornar erro ao tentar adiar projeto sem aviso', async () => {
      ...
      expect(postponeResult.error).toContain('Nenhum aviso de hibernação encontrado');
    });
  });
  ...
  describe('60.4: Atualização Automática e Feedback', () => {
    it('deve remover projeto da lista de inativos após adiamento', async () => {
      ...
      let projectBefore = activity.projectsWithActivity.find(p => p.id === projectId);
      ...
      let projectAfter = activity.projectsWithActivity.find(p => p.id === projectId);
    ...
    it('deve atualizar lastActivityAt para data atual após adiamento', async () => {
      ...
      expect(updatedDate.getTime()).toBeGreaterThanOrEqual(beforePostpone.getTime() - 1000);
      expect(updatedDate.getTime()).toBeLessThanOrEqual(afterPostpone.getTime() + 1000);

...
describe('Sistema de Geocodificação', () => {
  ...
  afterAll(async () => {
    ...
    if (testConcorrenteId) {
      await db.execute(`DELETE FROM concorrentes WHERE id = ${testConcorrenteId}`);
    }
    if (testLeadId) {
  ...
  describe('Funções de Banco de Dados', () => {
    ...
    it('deve buscar registros sem coordenadas filtrados por tipo', async () => {
      ...
      const concorrentes = await getRecordsSemCoordenadas(testProjectId, 'concorrente');
    ...
    it('deve atualizar coordenadas de cliente', async () => {
      ...
      const updated = await updateClienteCoordinates(testClienteId, latitude, longitude);
    ...
    it('deve atualizar coordenadas de concorrente', async () => {
      ...
      const updated = await updateConcorrenteCoordinates(testConcorrenteId, latitude, longitude);
      expect(updated).toBe(true);
    ...
    it('deve atualizar coordenadas de lead', async () => {
      ...
      const updated = await updateLeadCoordinates(testLeadId, latitude, longitude);
      expect(updated).toBe(true);

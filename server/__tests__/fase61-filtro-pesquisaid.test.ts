...
describe("Fase 61: Filtro por pesquisaId", () => {
  ...
  it("deve filtrar concorrentes por pesquisaId", async () => {
    ...
    expect(concorrentes1.some(c => c.nome === "Concorrente Pesquisa 1")).toBe(
      true
    );
    expect(concorrentes1.some(c => c.nome === "Concorrente Pesquisa 2")).toBe(
      false
    );
    ...
    expect(concorrentes2.some(c => c.nome === "Concorrente Pesquisa 2")).toBe(
      true
    );
    expect(concorrentes2.some(c => c.nome === "Concorrente Pesquisa 1")).toBe(
      false
    );
  });
  ...
  it("deve filtrar produtos por pesquisaId", async () => {
    // Filtrar por pesquisa 1
    const produtos1 = await getProdutosByProject(
      testProjectId,
      testPesquisa1Id
    );
    ...
    // Filtrar por pesquisa 2
    const produtos2 = await getProdutosByProject(
      testProjectId,
      testPesquisa2Id
    );

...
describe('Fase 61: Filtro por pesquisaId', () => {
  ...
  it('deve filtrar mercados por pesquisaId', async () => {
    ...
    console.log(`[TEST] Pesquisa1Id: ${testPesquisa1Id}, Mercados encontrados:`, mercados1.map(m => ({ id: m.id, nome: m.nome, pesquisaId: m.pesquisaId })));
  ...
  it('deve filtrar concorrentes por pesquisaId', async () => {
    // Filtrar por pesquisa 1
    const concorrentes1 = await getAllConcorrentes({ pesquisaId: testPesquisa1Id });
    ...
    // Filtrar por pesquisa 2
    const concorrentes2 = await getAllConcorrentes({ pesquisaId: testPesquisa2Id });

...
import { estimateFileSize, formatBytes } from '../services/export/fileSizeEstimator';
...
describe('Módulo de Exportação - Interpretation Service', () => {
  ...
  it('deve rejeitar entity inválida', () => {
    ...
    expect(result.valid).toBe(false);
    expect(result.errors).toContain('Entity inválida. Deve ser uma de: mercados, clientes, concorrentes, leads, produtos');
  });
});
...
describe('Módulo de Exportação - Query Builder', () => {
  ...
  it('deve executar query e retornar resultados', async () => {
    ...
    try {
      ...
      if (errorMsg.includes("doesn't exist") || errorMsg.includes('ER_NO_SUCH_TABLE')) {
...
describe('Módulo de Exportação - Excel Renderer', () => {
  ...
  it('deve retornar MIME type correto', () => {
    const renderer = new ExcelRenderer();
    expect(renderer.getMimeType()).toBe('application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
  });

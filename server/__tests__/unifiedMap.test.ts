...
describe('Sistema Unificado de Mapas - Queries tRPC', () => {
  ...
  beforeAll(async () => {
    ...
    if (db) {
      const projects = await db.execute(`SELECT id FROM projects WHERE ativo = 1 LIMIT 1`);
  ...
  describe('getAllEntities', () => {
    it('deve retornar array de entidades', async () => {
      const input: inferProcedureInput<typeof appRouter.unifiedMap.getAllEntities> = {
    ...
    it('deve retornar estrutura correta de entidade', async () => {
      ...
      if (result.length > 0) {
        ...
        expect(['mercado', 'cliente', 'produto', 'concorrente', 'lead']).toContain(entity.type);
      }
    });
  ...
  describe('getMapStats', () => {
    ...
    it('total deve ser soma de todos os tipos', async () => {
      ...
      const sumByType = Object.values(result.byType).reduce((sum, count) => sum + count, 0);

...
export async function checkAndSendAlerts(
  ...
  const errorRate = metrics.processedClients > 0 
  ...
  if (metrics.avgProcessingTime > config.processingTimeThreshold) {
    ...
    if (shouldSendAlert(alertKey)) {
      ...
      markAlertSent(alertKey);
      console.log(`[IntelligentAlerts] Processing time alert sent for project ${projectId}: ${metrics.avgProcessingTime}s`);
    }
  }
  ...
  if (metrics.isCompleted && config.notifyOnCompletion) {
    ...
    if (shouldSendAlert(alertKey)) {
      ...
      await notifyOwner({
        ...
        content: `O enriquecimento do projeto ${projectId} foi concluído com sucesso!\n\n` +
          `📊 Estatísticas:\n` +
      ...
      markAlertSent(alertKey);
      console.log(`[IntelligentAlerts] Completion alert sent for project ${projectId}`);
    }
  }
}
...
export function cleanAlertCache() {
  ...
  console.log(`[IntelligentAlerts] Cache cleaned: ${alertCache.size} entries remaining`);
}

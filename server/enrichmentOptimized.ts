...
export async function enrichClienteOptimized(
  ...
  try {
    ...
    if (allData.clienteEnriquecido) {
      ...
      if (enriched.cidade) {
        updateData.cidade = truncate(enriched.cidade, 100);
      }
      if (enriched.uf) {
        updateData.uf = truncate(enriched.uf, 2);
      }
      if (enriched.regiao) {
        updateData.regiao = truncate(enriched.regiao, 100);
      }
      if (enriched.porte) {
        updateData.porte = truncate(enriched.porte, 50);
      }
      if (enriched.email) {
        updateData.email = truncate(enriched.email, 320);
      }

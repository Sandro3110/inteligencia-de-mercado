    ...
    try {
        ...
        for (const mercadoItem of allData.mercados) {
            ...
            const [existingMercado] = await db
                ...
                .where((0, drizzle_orm_1.eq)(schema_1.mercadosUnicos.mercadoHash, mercadoHash))
                .limit(1);
            if (existingMercado) {
                ...
                const [newMercado] = await db
                    .insert(schema_1.mercadosUnicos)
                    .values({
                    projectId,
            ...
            for (const concorrenteData of mercadoItem.concorrentes) {
                ...
                const [existing] = await db
                    ...
                    .where((0, drizzle_orm_1.eq)(schema_1.concorrentes.concorrenteHash, concorrenteHash))
                    .limit(1);
                if (!existing) {
 ...
 */
async function enrichClientesParallel(clienteIds, projectId = 1, concurrency = 5, onProgress) {
    const results = [];
    let completed = 0;
    ...
    for (let i = 0; i < clienteIds.length; i += concurrency) {
        ...
        const batchResults = await Promise.all(batch.map(clienteId => enrichClienteOptimized(clienteId, projectId)));

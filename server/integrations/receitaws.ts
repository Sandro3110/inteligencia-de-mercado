 ...
 */
export async function validateCNPJ(cnpj: string): Promise<ReceitaWSResponse | null> {
  ...
  try {
    ...
    if (!response.ok) {
      console.warn(`[ReceitaWS] HTTP error ${response.status} for CNPJ ${cleanCNPJ}`);
      return null;
    }
    ...
    logger.debug(`[ReceitaWS] ✅ Validated CNPJ ${cleanCNPJ}: ${data.nome}`);
...
export async function validateCNPJsBatch(
  ...
  logger.debug(`[ReceitaWS] Validating ${cnpjs.length} CNPJs in batch...`);
  ...
  logger.debug(`[ReceitaWS] Validated ${results.size}/${cnpjs.length} CNPJs successfully`);

...
export function isRealCompany(result: {
  ...
  if (result.snippet) {
    ...
    if (cnpjs.length > 0) {
      console.log(`[Filter] CNPJ encontrado no snippet: ${cnpjs[0]} - Aprovado como empresa`);
      return true;
    }
  }
  ...
  console.log(`[Filter] Resultado rejeitado (sem CNPJ e sem domínio corporativo claro): ${result.title}`);
  return false;
}
...
function isLikelyCorporateDomain(domain: string): boolean {
  ...
  const contentKeywords = ['noticias', 'news', 'blog', 'portal', 'revista', 'jornal', 'guia', 'lista'];
...
export function filterRealCompanies<T extends { title: string; link: string; snippet?: string }>(
  ...
  console.log(`[Filter] ${filtered.length} empresas reais encontradas (${results.length - filtered.length} artigos removidos)`);

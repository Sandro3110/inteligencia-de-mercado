...
async function getUser(id) {
    ...
    const result = await db.select().from(schema_1.users).where((0, drizzle_orm_1.eq)(schema_1.users.id, id)).limit(1);
...
async function getDashboardStats(projectId) {
    ...
    const mercadosQuery = projectId
        ? db.select({ count: (0, drizzle_orm_1.count)() }).from(schema_1.mercadosUnicos).where((0, drizzle_orm_1.eq)(schema_1.mercadosUnicos.projectId, projectId))
        : db.select({ count: (0, drizzle_orm_1.count)() }).from(schema_1.mercadosUnicos);
    const [mercadosCount] = await mercadosQuery;
    const clientesQuery = projectId
        ? db.select({ count: (0, drizzle_orm_1.count)() }).from(schema_1.clientes).where((0, drizzle_orm_1.eq)(schema_1.clientes.projectId, projectId))
    ...
    const concorrentesQuery = projectId
        ? db.select({ count: (0, drizzle_orm_1.count)() }).from(schema_1.concorrentes).where((0, drizzle_orm_1.eq)(schema_1.concorrentes.projectId, projectId))
        : db.select({ count: (0, drizzle_orm_1.count)() }).from(schema_1.concorrentes);
    const [concorrentesCount] = await concorrentesQuery;
    const leadsQuery = projectId
        ? db.select({ count: (0, drizzle_orm_1.count)() }).from(schema_1.leads).where((0, drizzle_orm_1.eq)(schema_1.leads.projectId, projectId))
    ...
    if (projectId) {
        clientesStatusQuery.where((0, drizzle_orm_1.eq)(schema_1.clientes.projectId, projectId));
    }
    const clientesStatus = await clientesStatusQuery.groupBy(schema_1.clientes.validationStatus);
    ...
    if (projectId) {
        concorrentesStatusQuery.where((0, drizzle_orm_1.eq)(schema_1.concorrentes.projectId, projectId));
    }
    const concorrentesStatus = await concorrentesStatusQuery.groupBy(schema_1.concorrentes.validationStatus);
    const leadsStatusQuery = db
        ...
        .from(schema_1.leads);
    if (projectId) {
        leadsStatusQuery.where((0, drizzle_orm_1.eq)(schema_1.leads.projectId, projectId));
    }
    const leadsStatus = await leadsStatusQuery.groupBy(schema_1.leads.validationStatus);
    return {
        totals: {
...
async function getMercados(params) {
    ...
    const conditions = [];
    if (params?.projectId) {
        conditions.push((0, drizzle_orm_1.eq)(schema_1.mercadosUnicos.projectId, params.projectId));
    }
    if (params?.search) {
        conditions.push((0, drizzle_orm_1.or)((0, drizzle_orm_1.like)(schema_1.mercadosUnicos.nome, `%${params.search}%`), (0, drizzle_orm_1.like)(schema_1.mercadosUnicos.categoria, `%${params.search}%`)));
    }
    if (params?.categoria) {
        conditions.push((0, drizzle_orm_1.like)(schema_1.mercadosUnicos.categoria, `%${params.categoria}%`));
    }
    if (params?.segmentacao) {
        conditions.push((0, drizzle_orm_1.eq)(schema_1.mercadosUnicos.segmentacao, params.segmentacao));
    }
...
async function getAllClientes(params) {
    ...
    const conditions = [];
    if (params?.projectId) {
        conditions.push((0, drizzle_orm_1.eq)(schema_1.clientes.projectId, params.projectId));
    }
    if (params?.validationStatus) {
        conditions.push((0, drizzle_orm_1.eq)(schema_1.clientes.validationStatus, params.validationStatus));
    }
    if (conditions.length > 0) {
        return await db.select().from(schema_1.clientes).where((0, drizzle_orm_1.and)(...conditions));
    }
...
async function getClientesByMercado(mercadoId, validationStatus) {
    ...
    if (!db)
        return [];
    const conditions = [(0, drizzle_orm_1.eq)(schema_1.clientesMercados.mercadoId, mercadoId)];
    if (validationStatus) {
        conditions.push((0, drizzle_orm_1.eq)(schema_1.clientes.validationStatus, validationStatus));
    }
    return db
        .select({
        ...
        .from(schema_1.clientes)
        .innerJoin(schema_1.clientesMercados, (0, drizzle_orm_1.eq)(schema_1.clientes.id, schema_1.clientesMercados.clienteId))
        .where((0, drizzle_orm_1.and)(...conditions));
}
async function getClientesByMercadoPaginated(mercadoId, validationStatus, page = 1, pageSize = 20) {
    const db = await getDb();
    if (!db)
...
async function getAllConcorrentes(params) {
    ...
    const conditions = [];
    if (params?.projectId) {
        conditions.push((0, drizzle_orm_1.eq)(schema_1.concorrentes.projectId, params.projectId));
    }
    if (params?.validationStatus) {
        conditions.push((0, drizzle_orm_1.eq)(schema_1.concorrentes.validationStatus, params.validationStatus));
    }
    if (conditions.length > 0) {
        return await db.select().from(schema_1.concorrentes).where((0, drizzle_orm_1.and)(...conditions));
    }
...
async function getConcorrentesByMercado(mercadoId, validationStatus) {
    ...
    if (!db)
        return [];
    const conditions = [(0, drizzle_orm_1.eq)(schema_1.concorrentes.mercadoId, mercadoId)];
    if (validationStatus) {
        conditions.push((0, drizzle_orm_1.eq)(schema_1.concorrentes.validationStatus, validationStatus));
    }
    return db
        .select()
        .from(schema_1.concorrentes)
...
async function getConcorrentesByMercadoPaginated(mercadoId, validationStatus, page = 1, pageSize = 20) {
    const db = await getDb();
    if (!db)
        return { data: [], total: 0, page, pageSize, totalPages: 0 };
    const conditions = [(0, drizzle_orm_1.eq)(schema_1.concorrentes.mercadoId, mercadoId)];
...
async function getAllLeads(params) {
    ...
    const conditions = [];
    if (params?.projectId) {
        conditions.push((0, drizzle_orm_1.eq)(schema_1.leads.projectId, params.projectId));
    }
    if (params?.validationStatus) {
        conditions.push((0, drizzle_orm_1.eq)(schema_1.leads.validationStatus, params.validationStatus));
    }
    if (conditions.length > 0) {
        return await db.select().from(schema_1.leads).where((0, drizzle_orm_1.and)(...conditions));
    }
...
async function getLeadsByMercado(mercadoId, validationStatus) {
    ...
    if (!db)
        return [];
    const conditions = [(0, drizzle_orm_1.eq)(schema_1.leads.mercadoId, mercadoId)];
    if (validationStatus) {
        conditions.push((0, drizzle_orm_1.eq)(schema_1.leads.validationStatus, validationStatus));
    }
    return db
        .select()
        .from(schema_1.leads)
...
async function getLeadsByMercadoPaginated(mercadoId, validationStatus, page = 1, pageSize = 20) {
    const db = await getDb();
    if (!db)
        return { data: [], total: 0, page, pageSize, totalPages: 0 };
    const conditions = [(0, drizzle_orm_1.eq)(schema_1.leads.mercadoId, mercadoId)];
...
async function getAnalyticsProgress() {
    ...
    try {
        // Contar totais
        const [mercadosCount] = await db.select({ count: (0, drizzle_orm_1.sql) `COUNT(*)` }).from(schema_1.mercadosUnicos);
        const [clientesCount] = await db.select({ count: (0, drizzle_orm_1.sql) `COUNT(*)` }).from(schema_1.clientes);
        const [concorrentesCount] = await db.select({ count: (0, drizzle_orm_1.sql) `COUNT(*)` }).from(schema_1.concorrentes);
        const [leadsCount] = await db.select({ count: (0, drizzle_orm_1.sql) `COUNT(*)` }).from(schema_1.leads);
        // Contar por status (clientes + concorrentes + leads)
        const [clientesRich] = await db.select({ count: (0, drizzle_orm_1.sql) `COUNT(*)` })
            .from(schema_1.clientes)
            .where((0, drizzle_orm_1.eq)(schema_1.clientes.validationStatus, 'rich'));
        const [clientesPendentes] = await db.select({ count: (0, drizzle_orm_1.sql) `COUNT(*)` })
            .from(schema_1.clientes)
            .where((0, drizzle_orm_1.eq)(schema_1.clientes.validationStatus, 'pending'));
        const [clientesDiscarded] = await db.select({ count: (0, drizzle_orm_1.sql) `COUNT(*)` })
            .from(schema_1.clientes)
            .where((0, drizzle_orm_1.eq)(schema_1.clientes.validationStatus, 'discarded'));
        const [concorrentesRich] = await db.select({ count: (0, drizzle_orm_1.sql) `COUNT(*)` })
            .from(schema_1.concorrentes)
            .where((0, drizzle_orm_1.eq)(schema_1.concorrentes.validationStatus, 'rich'));
        const [concorrentesPendentes] = await db.select({ count: (0, drizzle_orm_1.sql) `COUNT(*)` })
            .from(schema_1.concorrentes)
            .where((0, drizzle_orm_1.eq)(schema_1.concorrentes.validationStatus, 'pending'));
        const [concorrentesDiscarded] = await db.select({ count: (0, drizzle_orm_1.sql) `COUNT(*)` })
            .from(schema_1.concorrentes)
            .where((0, drizzle_orm_1.eq)(schema_1.concorrentes.validationStatus, 'discarded'));
        const [leadsRich] = await db.select({ count: (0, drizzle_orm_1.sql) `COUNT(*)` })
            .from(schema_1.leads)
            .where((0, drizzle_orm_1.eq)(schema_1.leads.validationStatus, 'rich'));
        const [leadsPendentes] = await db.select({ count: (0, drizzle_orm_1.sql) `COUNT(*)` })
            .from(schema_1.leads)
            .where((0, drizzle_orm_1.eq)(schema_1.leads.validationStatus, 'pending'));
        const [leadsDiscarded] = await db.select({ count: (0, drizzle_orm_1.sql) `COUNT(*)` })
            .from(schema_1.leads)
            .where((0, drizzle_orm_1.eq)(schema_1.leads.validationStatus, 'discarded'));
        const totalRich = (clientesRich?.count || 0) +
            (concorrentesRich?.count || 0) +
            (leadsRich?.count || 0);
        const totalPendentes = (clientesPendentes?.count || 0) +
            (concorrentesPendentes?.count || 0) +
            (leadsPendentes?.count || 0);
        const totalDiscarded = (clientesDiscarded?.count || 0) +
        ...
        const progressoPorMercado = await db
            ...
            validados: (0, drizzle_orm_1.sql) `SUM(CASE WHEN ${schema_1.clientes.validationStatus} = 'validado' THEN 1 ELSE 0 END)`,
        })
            .from(schema_1.mercadosUnicos)
            .leftJoin(schema_1.clientesMercados, (0, drizzle_orm_1.eq)(schema_1.mercadosUnicos.id, schema_1.clientesMercados.mercadoId))
            .leftJoin(schema_1.clientes, (0, drizzle_orm_1.eq)(schema_1.clientesMercados.clienteId, schema_1.clientes.id))
            .groupBy(schema_1.mercadosUnicos.id, schema_1.mercadosUnicos.nome)
            .orderBy((0, drizzle_orm_1.sql) `COUNT(DISTINCT ${schema_1.clientes.id}) DESC`)
            .limit(10);
        return {
...
async function getAllTags() {
    ...
    if (!db)
        return [];
    const { tags } = await Promise.resolve().then(() => __importStar(require("../drizzle/schema")));
...
async function createTag(name, color = "#3b82f6") {
    ...
    const inserted = await db.select().from(tags).orderBy((0, drizzle_orm_1.sql) `id DESC`).limit(1);
...
async function getEntityTags(entityType, entityId) {
    ...
    if (!db)
        return [];
    const { entityTags, tags } = await Promise.resolve().then(() => __importStar(require("../drizzle/schema")));
    const results = await db
        ...
        .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(entityTags.entityType, entityType), (0, drizzle_orm_1.eq)(entityTags.entityId, entityId)));
    return results;
}
async function addTagToEntity(tagId, entityType, entityId) {
    ...
    if (!db)
        return null;
    const { entityTags } = await Promise.resolve().then(() => __importStar(require("../drizzle/schema")));
    // Check if already exists
    const existing = await db
        .select()
        .from(entityTags)
        .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(entityTags.tagId, tagId), (0, drizzle_orm_1.eq)(entityTags.entityType, entityType), (0, drizzle_orm_1.eq)(entityTags.entityId, entityId)))
        .limit(1);
...
async function getEntitiesByTag(tagId, entityType) {
    ...
    const results = await db
        .select()
        .from(entityTags)
        .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(entityTags.tagId, tagId), (0, drizzle_orm_1.eq)(entityTags.entityType, entityType)));
...
async function getSavedFilters(userId) {
    ...
    if (!db)
        return [];
    const { savedFilters } = await Promise.resolve().then(() => __importStar(require("../drizzle/schema")));
...
async function getDistribuicaoGeografica() {
    ...
    if (!db)
        return [];
    const { clientes } = await Promise.resolve().then(() => __importStar(require("../drizzle/schema")));
...
async function getFunilConversao() {
    ...
    const { leads, clientes } = await Promise.resolve().then(() => __importStar(require("../drizzle/schema")));
    const { sql, count } = await Promise.resolve().then(() => __importStar(require("drizzle-orm")));
...
async function getTop10Mercados() {
    ...
    if (!db)
        return [];
    const { mercadosUnicos } = await Promise.resolve().then(() => __importStar(require("../drizzle/schema")));
...
async function createMercado(data) {
    ...
    const existing = await db.select().from(schema_1.mercadosUnicos)
        .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.mercadosUnicos.mercadoHash, mercadoHash), (0, drizzle_orm_1.eq)(schema_1.mercadosUnicos.projectId, data.projectId)))
        .limit(1);
    if (existing.length > 0) {
        // Detectar mudanças
        const { detectChanges, trackMercadoChanges } = await Promise.resolve().then(() => __importStar(require('./_core/historyTracker')));
        const changes = detectChanges(existing[0], data, ['nome', 'categoria', 'segmentacao', 'tamanhoMercado', 'crescimentoAnual', 'tendencias', 'principaisPlayers']);
        // Registrar histórico
        await trackMercadoChanges(existing[0].id, changes, 'updated');
        // Atualizar se houver mudanças
        if (changes.length > 0) {
            await db.update(schema_1.mercadosUnicos)
                .set({
                nome: data.nome,
                ...
                .where((0, drizzle_orm_1.eq)(schema_1.mercadosUnicos.id, existing[0].id));
    ...
    const { trackCreation } = await Promise.resolve().then(() => __importStar(require('./_core/historyTracker')));
...
async function deleteMercado(id) {
    ...
    await db.delete(schema_1.clientesMercados).where((0, drizzle_orm_1.eq)(schema_1.clientesMercados.mercadoId, id));
    await db.delete(schema_1.concorrentes).where((0, drizzle_orm_1.eq)(schema_1.concorrentes.mercadoId, id));
    await db.delete(schema_1.leads).where((0, drizzle_orm_1.eq)(schema_1.leads.mercadoId, id));
    // Delete the mercado itself
    await db.delete(schema_1.mercadosUnicos).where((0, drizzle_orm_1.eq)(schema_1.mercadosUnicos.id, id));
    return true;
}
// ============================================
// CRUD - CLIENTES
...
async function createCliente(data) {
    ...
    const existing = await db.select().from(schema_1.clientes)
        .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.clientes.clienteHash, normalizedHash), (0, drizzle_orm_1.eq)(schema_1.clientes.projectId, data.projectId)))
        .limit(1);
    if (existing.length > 0) {
        // Detectar mudanças
        const { detectChanges, trackClienteChanges } = await Promise.resolve().then(() => __importStar(require('./_core/historyTracker')));
        const changes = detectChanges(existing[0], data, ['nome', 'cnpj', 'siteOficial', 'produtoPrincipal', 'email', 'telefone', 'cidade', 'uf', 'linkedin', 'instagram', 'cnae', 'porte']);
        // Registrar histórico
    ...
    if (!result.insertId)
        return null;
    const [cliente] = await db.select().from(schema_1.clientes).where((0, drizzle_orm_1.eq)(schema_1.clientes.id, Number(result.insertId)));
    // Registrar criação no histórico
    const { trackCreation } = await Promise.resolve().then(() => __importStar(require('./_core/historyTracker')));
...
async function updateCliente(id, data) {
    ...
    await db.update(schema_1.clientes)
        .set(updateData)
        .where((0, drizzle_orm_1.eq)(schema_1.clientes.id, id));
    const [cliente] = await db.select().from(schema_1.clientes).where((0, drizzle_orm_1.eq)(schema_1.clientes.id, id));
    return cliente;
}
async function deleteCliente(id) {
    ...
    await db.delete(schema_1.clientesMercados).where((0, drizzle_orm_1.eq)(schema_1.clientesMercados.clienteId, id));
    // Delete the cliente itself
    await db.delete(schema_1.clientes).where((0, drizzle_orm_1.eq)(schema_1.clientes.id, id));
    return true;
}
// ============================================
// CRUD - CONCORRENTES
...
async function createConcorrente(data) {
    ...
    const concorrenteHash = `${data.nome}-${data.mercadoId}-${data.projectId}`
        ...
        .replace(/^-|-$/g, '');
    // Verificar se já existe
    const existing = await db.select().from(schema_1.concorrentes)
        .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.concorrentes.concorrenteHash, concorrenteHash), (0, drizzle_orm_1.eq)(schema_1.concorrentes.projectId, data.projectId)))
        .limit(1);
    if (existing.length > 0) {
        // Detectar mudanças
        const { detectChanges, trackConcorrenteChanges } = await Promise.resolve().then(() => __importStar(require('./_core/historyTracker')));
        const changes = detectChanges(existing[0], data, ['nome', 'cnpj', 'site', 'produto', 'porte', 'faturamentoEstimado']);
        // Registrar histórico
        await trackConcorrenteChanges(existing[0].id, changes, 'enriched');
        // Atualizar se houver mudanças
        if (changes.length > 0) {
            await db.update(schema_1.concorrentes)
                .set({
                nome: data.nome,
                cnpj: data.cnpj || existing[0].cnpj,
    ...
    if (!result.insertId)
        return null;
    const [concorrente] = await db.select().from(schema_1.concorrentes).where((0, drizzle_orm_1.eq)(schema_1.concorrentes.id, Number(result.insertId)));
    // Registrar criação no histórico
    const { trackCreation } = await Promise.resolve().then(() => __importStar(require('./_core/historyTracker')));
...
async function updateConcorrente(id, data) {
    ...
    await db.update(schema_1.concorrentes)
        .set(updateData)
        .where((0, drizzle_orm_1.eq)(schema_1.concorrentes.id, id));
    const [concorrente] = await db.select().from(schema_1.concorrentes).where((0, drizzle_orm_1.eq)(schema_1.concorrentes.id, id));
    return concorrente;
}
async function deleteConcorrente(id) {
    ...
    if (!db)
        return false;
    await db.delete(schema_1.concorrentes).where((0, drizzle_orm_1.eq)(schema_1.concorrentes.id, id));
    return true;
}
// ============================================
// CRUD - LEADS
...
async function createLead(data) {
    ...
    const leadHash = `${data.nome}-${data.mercadoId}-${data.projectId}`
        ...
        .replace(/^-|-$/g, '');
    // Verificar se já existe
    const existing = await db.select().from(schema_1.leads)
        .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.leads.leadHash, leadHash), (0, drizzle_orm_1.eq)(schema_1.leads.projectId, data.projectId)))
        .limit(1);
    if (existing.length > 0) {
        // Detectar mudanças (NÃO incluir 'stage' para preservar progresso de vendas)
        const { detectChanges, trackLeadChanges } = await Promise.resolve().then(() => __importStar(require('./_core/historyTracker')));
        const changes = detectChanges(existing[0], data, ['nome', 'cnpj', 'site', 'email', 'telefone', 'tipo', 'porte', 'regiao', 'setor']);
        // Registrar histórico
        await trackLeadChanges(existing[0].id, changes, 'enriched');
        // Atualizar se houver mudanças (sem modificar stage)
        if (changes.length > 0) {
            await db.update(schema_1.leads)
                .set({
                nome: data.nome,
                cnpj: data.cnpj || existing[0].cnpj,
                site: data.site || existing[0].site,
  ...
  `);
    if (!result.insertId)
        return null;
    const [lead] = await db.select().from(schema_1.leads).where((0, drizzle_orm_1.eq)(schema_1.leads.id, Number(result.insertId)));
    // Registrar criação no histórico
    const { trackCreation } = await Promise.resolve().then(() => __importStar(require('./_core/historyTracker')));
...
async function updateLead(id, data) {
    ...
    await db.update(schema_1.leads)
        .set(updateData)
        .where((0, drizzle_orm_1.eq)(schema_1.leads.id, id));
    const [lead] = await db.select().from(schema_1.leads).where((0, drizzle_orm_1.eq)(schema_1.leads.id, id));
    return lead;
}
async function deleteLead(id) {
    ...
    if (!db)
        return false;
    await db.delete(schema_1.leads).where((0, drizzle_orm_1.eq)(schema_1.leads.id, id));
    return true;
}
// ============================================
...
async function getAllTemplates() {
    ...
    if (!db)
        return [];
    return await db.select().from(schema_1.projectTemplates).orderBy(schema_1.projectTemplates.isDefault, schema_1.projectTemplates.name);
}
async function getTemplateById(id) {
    const db = await getDb();
    if (!db)
        return null;
    const [template] = await db.select().from(schema_1.projectTemplates).where((0, drizzle_orm_1.eq)(schema_1.projectTemplates.id, id));
...
async function updateTemplate(id, data) {
    ...
    if (Object.keys(updateData).length > 0) {
        await db.update(schema_1.projectTemplates)
            .set(updateData)
...
async function deleteTemplate(id) {
    ...
    await db.delete(schema_1.projectTemplates).where((0, drizzle_orm_1.eq)(schema_1.projectTemplates.id, id));
    return true;
}
// ============================================
...
async function searchLeadsAdvanced(projectId, filter, page = 1, pageSize = 20) {
    ...
    const { buildDynamicQuery, validateFilter } = await Promise.resolve().then(() => __importStar(require('./queryBuilder')));
    // Validar filtro
    ...
    const finalWhere = whereClause
        ? (0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.leads.projectId, projectId), whereClause)
        : (0, drizzle_orm_1.eq)(schema_1.leads.projectId, projectId);
    // Base query
    const query = db.select().from(schema_1.leads).where(finalWhere);
    // Contar total
    const countQuery = db.select({ count: (0, drizzle_orm_1.count)() }).from(schema_1.leads).where(whereClause
        ? (0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.leads.projectId, projectId), whereClause)
        : (0, drizzle_orm_1.eq)(schema_1.leads.projectId, projectId));
...
async function createNotification(data) {
    ...
    if (!result.insertId)
        return null;
    const [notification] = await db.select().from(schema_1.notifications).where((0, drizzle_orm_1.eq)(schema_1.notifications.id, Number(result.insertId)));
    return notification;
}
...
async function getUnreadNotificationsCount(userId) {
    ...
    if (!db)
        return 0;
    const result = await db.select({ count: (0, drizzle_orm_1.count)() })
        .from(schema_1.notifications)
        .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.notifications.userId, userId), (0, drizzle_orm_1.eq)(schema_1.notifications.isRead, 0)));
...
async function markNotificationAsRead(id) {
    ...
    if (!db)
        return false;
    await db.update(schema_1.notifications)
        .set({ isRead: 1 })
...
async function deleteNotification(id) {
    ...
    if (!db)
        return false;
    await db.delete(schema_1.notifications).where((0, drizzle_orm_1.eq)(schema_1.notifications.id, id));
    return true;
}
// ============================================
...
async function checkAndNotifyHighQualityLead(leadId, qualityScore, userId) {
    if (qualityScore >= 80) {
        ...
        if (!db)
            return;
        const [lead] = await db.select().from(schema_1.leads).where((0, drizzle_orm_1.eq)(schema_1.leads.id, leadId));
        if (!lead)
            return;
...
async function notifyNewCompetitor(concorrenteId, userId) {
    ...
    if (!db)
        return;
    const [concorrente] = await db.select().from(schema_1.concorrentes).where((0, drizzle_orm_1.eq)(schema_1.concorrentes.id, concorrenteId));
    if (!concorrente)
        return;
    await createNotification({
        userId,
...
async function getEnrichmentProgress(projectId) {
    ...
    const [processedResult] = await db
        .select({ count: (0, drizzle_orm_1.sql) `COUNT(DISTINCT ${schema_1.clientes.id})` })
        .from(schema_1.clientes)
        .innerJoin(schema_1.clientesMercados, (0, drizzle_orm_1.eq)(schema_1.clientesMercados.clienteId, schema_1.clientes.id))
        .where((0, drizzle_orm_1.eq)(schema_1.clientes.projectId, projectId));
    const processed = Number(processedResult?.count || 0);
    // Mercados únicos do projeto
    const [mercadosResult] = await db
        .select({ count: (0, drizzle_orm_1.sql) `COUNT(DISTINCT ${schema_1.mercadosUnicos.id})` })
        .from(schema_1.mercadosUnicos)
        .innerJoin(schema_1.clientesMercados, (0, drizzle_orm_1.eq)(schema_1.clientesMercados.mercadoId, schema_1.mercadosUnicos.id))
        .innerJoin(schema_1.clientes, (0, drizzle_orm_1.eq)(schema_1.clientes.id, schema_1.clientesMercados.clienteId))
        .where((0, drizzle_orm_1.eq)(schema_1.clientes.projectId, projectId));
...
async function createEnrichmentRun(projectId, totalClients) {
    ...
    const { enrichmentRuns } = await Promise.resolve().then(() => __importStar(require("../drizzle/schema")));
...
async function getActiveEnrichmentRun(projectId) {
    ...
    const result = await db
        .select()
        .from(enrichmentRuns)
        .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(enrichmentRuns.projectId, projectId), (0, drizzle_orm_1.sql) `${enrichmentRuns.status} IN ('running', 'paused')`))
        .orderBy((0, drizzle_orm_1.sql) `${enrichmentRuns.startedAt} DESC`)
        .limit(1);
    return result.length > 0 ? result[0] : null;
}
...
async function listScheduledEnrichments(projectId) {
    ...
    if (!db)
        return [];
    return await db.select().from(schema_2.scheduledEnrichments)
        .where((0, drizzle_orm_1.eq)(schema_2.scheduledEnrichments.projectId, projectId))
...
async function cancelScheduledEnrichment(id) {
    ...
    if (!db)
        throw new Error("Database not available");
    await db.update(schema_2.scheduledEnrichments)
...
async function deleteScheduledEnrichment(id) {
    ...
    if (!db)
        throw new Error("Database not available");
    await db.delete(schema_2.scheduledEnrichments).where((0, drizzle_orm_1.eq)(schema_2.scheduledEnrichments.id, id));
}
...
async function getAlertHistory(projectId, options) {
    ...
    if (!db)
        return [];
    const conditions = [(0, drizzle_orm_1.eq)(schema_3.alertHistory.projectId, projectId)];
    if (options?.alertType) {
        conditions.push((0, drizzle_orm_1.eq)(schema_3.alertHistory.alertType, options.alertType));
    }
    const results = await db
        .select()
...
async function calculateROIMetrics(projectId) {
    ...
    const conversionsResult = await db
        .select({ count: (0, drizzle_orm_1.sql) `COUNT(*)` })
        .from(schema_3.leadConversions)
        .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_3.leadConversions.projectId, projectId), (0, drizzle_orm_1.eq)(schema_3.leadConversions.status, 'won')));
    ...
    const conversionsByMarketResult = await db
        ...
        mercadoNome: schema_1.mercadosUnicos.nome,
        conversions: (0, drizzle_orm_1.sql) `COUNT(${schema_3.leadConversions.id})`,
        totalValue: (0, drizzle_orm_1.sql) `SUM(${schema_3.leadConversions.dealValue})`,
    })
        .from(schema_3.leadConversions)
        .innerJoin(schema_1.leads, (0, drizzle_orm_1.eq)(schema_3.leadConversions.leadId, schema_1.leads.id))
        .innerJoin(schema_1.mercadosUnicos, (0, drizzle_orm_1.eq)(schema_1.leads.mercadoId, schema_1.mercadosUnicos.id))
        .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_3.leadConversions.projectId, projectId), (0, drizzle_orm_1.eq)(schema_3.leadConversions.status, 'won')))
        .groupBy(schema_1.leads.mercadoId, schema_1.mercadosUnicos.nome)
        .orderBy((0, drizzle_orm_1.desc)((0, drizzle_orm_1.sql) `COUNT(${schema_3.leadConversions.id})`));
    return {
        totalLeads,
        totalConversions,
        totalDealValue,
...
async function globalSearch(query, projectId, limit = 20) {
    ...
    try {
        // Buscar mercados
        const mercadosConditions = [(0, drizzle_orm_1.sql) `${schema_1.mercadosUnicos.nome} LIKE ${searchTerm}`];
        if (projectId) {
            mercadosConditions.push((0, drizzle_orm_1.eq)(schema_1.mercadosUnicos.projectId, projectId));
        }
        ...
        const clientesConditions = [
            (0, drizzle_orm_1.or)((0, drizzle_orm_1.sql) `${schema_1.clientes.nome} LIKE ${searchTerm}`, (0, drizzle_orm_1.sql) `${schema_1.clientes.cnpj} LIKE ${searchTerm}`)
        ];
        if (projectId) {
            clientesConditions.push((0, drizzle_orm_1.eq)(schema_1.clientes.projectId, projectId));
        }
        ...
        const concorrentesConditions = [
            (0, drizzle_orm_1.or)((0, drizzle_orm_1.sql) `${schema_1.concorrentes.nome} LIKE ${searchTerm}`, (0, drizzle_orm_1.sql) `${schema_1.concorrentes.cnpj} LIKE ${searchTerm}`)
        ];
        if (projectId) {
            concorrentesConditions.push((0, drizzle_orm_1.eq)(schema_1.concorrentes.projectId, projectId));
        }
        const concorrentesResult = await db
            .select({
        ...
        const leadsConditions = [
            (0, drizzle_orm_1.or)((0, drizzle_orm_1.sql) `${schema_1.leads.nome} LIKE ${searchTerm}`, (0, drizzle_orm_1.sql) `${schema_1.leads.email} LIKE ${searchTerm}`, (0, drizzle_orm_1.sql) `${schema_1.leads.telefone} LIKE ${searchTerm}`)
        ];
        if (projectId) {
            leadsConditions.push((0, drizzle_orm_1.eq)(schema_1.leads.projectId, projectId));
        }
        const leadsResult = await db
            .select({
...
async function getProdutosByCliente(clienteId) {
    ...
    if (!db)
        return [];
    try {
        const result = await db.select().from(schema_1.produtos)
...
async function updateProduto(id, data) {
    ...
    if (!db)
        return null;
    try {
        await db.update(schema_1.produtos)
            .set({
            ...data,
            updatedAt: new Date(),
        })
...
async function deleteProduto(id) {
    ...
    if (!db)
        return false;
    try {
        await db.delete(schema_1.produtos).where((0, drizzle_orm_1.eq)(schema_1.produtos.id, id));
        return true;
    }
    catch (error) {

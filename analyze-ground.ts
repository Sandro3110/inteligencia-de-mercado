...
import { projects, pesquisas, clientes, mercadosUnicos, concorrentes, leads, produtos, enrichmentJobs } from './drizzle/schema';
...
async function analyzeGround() {
  ...
  const projectList = await db.select().from(projects).where(sql`name LIKE '%Ground%' OR name LIKE '%ground%'`);
  ...
  // 2. Buscar pesquisas
  const pesquisasList = await db.select().from(pesquisas).where(eq(pesquisas.projectId, project.id));
  ...
  const clientesList = await db.select().from(clientes).where(eq(clientes.projectId, project.id)).limit(10);
  const clientesTotal = await db.select({ count: sql<number>`count(*)` }).from(clientes).where(eq(clientes.projectId, project.id));
  ...
  const mercadosList = await db.select().from(mercadosUnicos).where(eq(mercadosUnicos.projectId, project.id)).limit(10);
  const mercadosTotal = await db.select({ count: sql<number>`count(*)` }).from(mercadosUnicos).where(eq(mercadosUnicos.projectId, project.id));
  ...
  const concorrentesTotal = await db.select({ count: sql<number>`count(*)` }).from(concorrentes).where(eq(concorrentes.projectId, project.id));
  const leadsTotal = await db.select({ count: sql<number>`count(*)` }).from(leads).where(eq(leads.projectId, project.id));
  const produtosTotal = await db.select({ count: sql<number>`count(*)` }).from(produtos).where(eq(produtos.projectId, project.id));
  ...
  }).from(clientes).where(eq(clientes.projectId, project.id));
  ...
  const jobsList = await db.select().from(enrichmentJobs)
    .where(sql`pesquisaId IN (SELECT id FROM pesquisas WHERE projectId = ${project.id})`)
  ...
  const concorrentesPorCliente = clientesCount > 0 ? concorrentesTotal[0].count / clientesCount : 0;
  const leadsPorCliente = clientesCount > 0 ? leadsTotal[0].count / clientesCount : 0;
  const produtosPorCliente = clientesCount > 0 ? produtosTotal[0].count / clientesCount : 0;

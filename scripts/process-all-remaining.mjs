...
async function main() {
  ...
  for (let i = 0; i < clientesPendentes.length; i++) {
    ...
    const porcentagem = ((stats.processados / stats.totalClientes) * 100).toFixed(1);
    ...
    try {
      ...
      if (stats.processados % 100 === 0) {
        console.log('\n' + '='.repeat(70));
        console.log(`📊 CHECKPOINT ${stats.processados}/${stats.totalClientes}`);
        console.log(`   ✅ Sucessos: ${stats.sucessos} | ❌ Erros: ${stats.erros}`);
        console.log(`   Taxa: ${((stats.sucessos / stats.processados) * 100).toFixed(1)}%`);
...
async function gerarRelatorioFinal() {
  ...
  const tempoMedio = stats.tempos.reduce((a, b) => a + b, 0) / stats.tempos.length;

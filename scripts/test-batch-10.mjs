...
async function main() {
  ...
  const [mercadosCount] = await db.select({ count: sql`COUNT(DISTINCT id)` }).from(mercadosUnicos).where(eq(mercadosUnicos.projectId, PROJECT_ID));
  const [produtosCount] = await db.select({ count: sql`COUNT(*)` }).from(produtos).where(eq(produtos.projectId, PROJECT_ID));
  const [concorrentesCount] = await db.select({ count: sql`COUNT(DISTINCT concorrenteHash)` }).from(concorrentes).where(eq(concorrentes.projectId, PROJECT_ID));
  const [leadsCount] = await db.select({ count: sql`COUNT(DISTINCT leadHash)` }).from(leads).where(eq(leads.projectId, PROJECT_ID));
  ...
  console.log('   - Checkpoint automático a cada 50 clientes (neste teste não será atingido)');
  console.log('   - Notificações enviadas ao proprietário em cada checkpoint\n');

  console.log('======================================================================');
  console.log('✅ Preparação concluída! Acesse a interface para iniciar o teste.');
}

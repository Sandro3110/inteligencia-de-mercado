 ...
 * Usa o MCP CLI para buscar os dados
 */

import { execSync } from 'child_process';
import { writeFileSync, readFileSync } from 'fs';
import { drizzle } from 'drizzle-orm/mysql2';
import { mercadosUnicos, clientes, clientesMercados, concorrentes, leads } from '../drizzle/schema.ts';
...
function querySupabase(sql) {
  ...
  if (!match) {
    console.error('❌ Erro ao extrair dados:', result);
    return [];
  }
...
async function migrate() {
  try {
    ...
    const mercadosData = querySupabase('SELECT * FROM mercados_unicos ORDER BY id');
    ...
    for (const cliente of clientesData) {
      ...
      }).onDuplicateKeyUpdate({ set: { nome: cliente.nome || cliente.empresa } });
    }
    ...
    const concorrentesData = querySupabase('SELECT * FROM concorrentes_fase3 ORDER BY id');
    ...
    console.log(`   - Total: ${mercadosData.length + clientesData.length + cmData.length + concorrentesData.length + leadsData.length} registros\n`);

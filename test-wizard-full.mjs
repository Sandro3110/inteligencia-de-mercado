...
(async () => {
  ...
  await page.goto('http://localhost:3000/research/new', { waitUntil: 'networkidle' });
  ...
  const projectSelected = await page.locator('text=✓ Projeto selecionado').count();
  console.log('- Projeto selecionado:', projectSelected > 0 ? '✅ SIM' : '❌ NÃO');
  
  // Clicar em Próximo
  const nextBtn = page.locator('button:has-text("Próximo")');
  const isNextEnabled = await nextBtn.isEnabled();
  console.log('- Botão Próximo habilitado:', isNextEnabled ? '✅ SIM' : '❌ NÃO');
  ...
  if (isNextEnabled) {
    ...
    if (step2Title > 0) {
      ...
      console.log('- Botão Próximo habilitado:', isNext2Enabled ? '✅ SIM' : '❌ NÃO');
      ...
      if (isNext2Enabled) {
        ...
        const step3Title = await page.locator('text=Configurar Parâmetros').count();
        console.log('- Avançou para Step 3:', step3Title > 0 ? '✅ SIM' : '❌ NÃO');
      }
    }
  }
  
  // Screenshot final
  await page.screenshot({ path: '/home/ubuntu/screenshots/wizard-final.png', fullPage: true });

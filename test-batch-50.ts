...
import {
  ...
  BatchProgress,
  BatchResult,
} from "./server/enrichmentBatchProcessor";
...
const onProgress = (progress: BatchProgress) => {
  ...
  process.stdout.write(
    `\r⏳ Progresso: ${progress.processados}/${progress.totalClientes} (${progress.percentual}%) - Bloco ${progress.blocoAtual}/${progress.totalBlocos}`
  );
};
...
try {
  ...
  if (lastProgress) {
    ...
    console.log(
      `   Sucessos: ${lastProgress.sucessos} (${Math.round((lastProgress.sucessos / lastProgress.processados) * 100)}%)`
    );
    console.log(
      `   Erros: ${lastProgress.erros} (${Math.round((lastProgress.erros / lastProgress.processados) * 100)}%)`
    );
    console.log(
      `   Tempo total: ${Math.round(totalDuration / 1000)}s (~${Math.round(totalDuration / 60000)} minutos)`
    );
    console.log(
      `   Tempo médio por cliente: ${Math.round(totalDuration / lastProgress.processados / 1000)}s`
    );
  }
  ...
  if (batchResults.length > 0) {
    ...
    batchResults.forEach(result => {
      ...
      if (result.clientesComErro.length > 0) {
        console.log(
          `     - IDs com erro: ${result.clientesComErro.join(", ")}`
        );
      }
    });
  }

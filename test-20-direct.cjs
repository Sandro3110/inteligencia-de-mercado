...
async function enrichClienteCompleto(clienteId, projectId, connection) {
  ...
  for (const conc of concorrentes) {
    const concHash = require("crypto")
      .createHash("md5")
      .update(conc.nome + conc.cnpj)
      .digest("hex");
  ...
  for (const lead of leads) {
    const leadHash = require("crypto")
      .createHash("md5")
      .update(lead.nome + lead.cnpj)
      .digest("hex");
...
async function main() {
  ...
  for (let i = 0; i < clientes.length; i++) {
    ...
    try {
      ...
      console.log(`   ✅ Sucesso!`);
      console.log(
        `      └─ ${result.mercados.length} mercados, ${result.produtos.length} produtos, ${result.concorrentes.length} concorrentes, ${result.leads.length} leads`
      );
    } catch (error) {
      results.errors++;
  ...
  const resultsJSON = {
    ...
    successRate: parseFloat(
      ((results.success / clientes.length) * 100).toFixed(1)
    ),
    summary: {
  ...
  await fs.writeFile(
    "/home/ubuntu/TESTE_20_RESULTS.json",

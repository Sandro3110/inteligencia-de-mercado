...
async function main() {
  ...
  console.log(
    "║    TESTE COMPARATIVO: OTIMIZADO (1 call) vs ANTIGO (13 calls) ║"
  );
  console.log(
    "╚════════════════════════════════════════════════════════════════╝\n"
  );
  ...
  try {
    ...
    clientes.forEach((c: any, i: number) => {
      ...
      console.log(
        `      Produto: ${c.produtoPrincipal?.substring(0, 60) || "N/A"}...`
      );
    });
    console.log("");
    ...
    console.log(
      "🚀 Iniciando enriquecimento OTIMIZADO (Paralelo, 1 call OpenAI)...\n"
    );
    ...
    console.log(
      `   - Concorrentes: ${stats[0].avgScoreConcorrentes?.toFixed(1) || "N/A"}/100`
    );
    console.log(
      `   - Leads: ${stats[0].avgScoreLeads?.toFixed(1) || "N/A"}/100`
    );
    console.log(`\n🎯 CNPJs Encontrados:`);
    console.log(
      `   - Concorrentes: ${stats[0].concorrentesComCNPJ}/${totalConcorrentes} (${((stats[0].concorrentesComCNPJ / totalConcorrentes) * 100).toFixed(0)}%)`
    );
    console.log(
      `   - Leads: ${stats[0].leadsComCNPJ}/${totalLeads} (${((stats[0].leadsComCNPJ / totalLeads) * 100).toFixed(0)}%)`
    );
    ...
    console.log(
      `   📊 Registros Esperados: ~${((totalMercados + totalProdutos + totalConcorrentes + totalLeads) * 801) / 5} registros`
    );
    ...
    console.log(
      `│ Métrica             │ OTIMIZADO    │ ANTIGO       │ Melhoria     │`
    );
    console.log(
      `├─────────────────────┼──────────────┼──────────────┼──────────────┤`
    );
    console.log(
      `│ Tempo/Cliente       │ ${avgDuration.toFixed(1).padEnd(12)} │ 150s         │ ${(150 / avgDuration).toFixed(1)}x mais rápido │`
    );
    console.log(
      `│ Chamadas OpenAI     │ 1            │ 10-13        │ 10-13x menos │`
    );
    console.log(
      `│ Chamadas SerpAPI    │ 0            │ 45           │ 100% menos   │`
    );
    console.log(
      `│ Tempo Total (801)   │ ${(projectedTime / 60).toFixed(0)}min        │ 2000min      │ ${(2000 / (projectedTime / 60)).toFixed(0)}x mais rápido │`
    );
    console.log(
      `│ Custo (801)         │ $${projectedCost.toFixed(2)}        │ $72          │ ${(72 / projectedCost).toFixed(0)}x mais barato│`
    );
    console.log(
      `└─────────────────────┴──────────────┴──────────────┴──────────────┘`
    );
    ...
    fs.writeFileSync(
      "/home/ubuntu/TEST_OPTIMIZED_REPORT.json",

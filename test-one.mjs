...
async function main() {
  ...
  const [clienteEnriquecido] = await db
    .select()
    .from(clientes)
  ...
  const produtosResult = await db
    .select()
    .from(produtos)
    .where(eq(produtos.clienteId, cliente.id));
  ...
  console.log(
    `   Cidade/UF: ${clienteEnriquecido.cidade || "?"}/${clienteEnriquecido.uf || "?"}`
  );
  console.log(`   Região: ${clienteEnriquecido.regiao || "N/A"}`);
  console.log(`   Porte: ${clienteEnriquecido.porte || "N/A"}`);
  console.log(
    `   Faturamento: ${clienteEnriquecido.faturamentoDeclarado || "N/A"}`
  );
  console.log(
    `   Quality Score: ${clienteEnriquecido.qualidadeScore || 0}/100\n`
  );

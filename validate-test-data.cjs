...
async function main() {
  ...
  console.log(
    "║           VALIDAÇÃO DE DADOS - TESTE 20 CLIENTES              ║"
  );
  console.log(
    "╚════════════════════════════════════════════════════════════════╝\n"
  );
  ...
  const testResults = JSON.parse(
    fs.readFileSync("/home/ubuntu/TESTE_20_RESULTS.json", "utf8")
  );
  ...
  validation.clientes_mercados.ok =
    validation.clientes_mercados.found >= validation.clientes_mercados.expected;

  console.log(
    `   Esperado: ${validation.clientes_mercados.expected} registros`
  );
  console.log(`   Encontrado: ${validation.clientes_mercados.found} registros`);
  console.log(
    `   Status: ${validation.clientes_mercados.ok ? "✅ OK" : "❌ FALHA"}\n`
  );
  ...
  validation.produtos.ok =
    validation.produtos.found >= validation.produtos.expected;
  ...
  validation.concorrentes.ok =
    validation.concorrentes.found >= validation.concorrentes.expected;
  ...
  console.log(
    `   Status: ${validation.concorrentes.ok ? "✅ OK" : "❌ FALHA"}\n`
  );
  ...
  validation.mercados_unicos.ok =
    validation.mercados_unicos.found >= validation.mercados_unicos.expected;
  ...
  console.log(
    `   Status: ${validation.mercados_unicos.ok ? "✅ OK" : "❌ FALHA"}\n`
  );
  ...
  tabelas.forEach(tabela => {
    ...
    if (v.ok) okCount++;
    console.log(
      `${v.ok ? "✅" : "❌"} ${tabela.padEnd(25)} ${v.found}/${v.expected}`
    );
  });
  ...
  console.log(
    `1. Produtos órfãos (sem cliente ou mercado válido): ${orphanProds[0].count}`
  );
  console.log(
    `   Status: ${orphanProds[0].count === 0 ? "✅ OK" : "❌ FALHA"}\n`
  );
  ...
  console.log(
    `2. Concorrentes órfãos (sem mercado válido): ${orphanConc[0].count}`
  );
  console.log(
    `   Status: ${orphanConc[0].count === 0 ? "✅ OK" : "❌ FALHA"}\n`
  );
  ...
  console.log(
    `   Status: ${orphanLeads[0].count === 0 ? "✅ OK" : "❌ FALHA"}\n`
  );

  const integrityOk =
    orphanProds[0].count === 0 &&
    orphanConc[0].count === 0 &&
    orphanLeads[0].count === 0;

  console.log(
    `📊 Integridade Referencial: ${integrityOk ? "✅ APROVADA" : "❌ REPROVADA"}\n`
  );
  ...
  fs.writeFileSync(
    "/home/ubuntu/VALIDATION_RESULTS.json",

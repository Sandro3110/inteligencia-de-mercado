...
async function geocodeClientes(limit = null) {
  ...
  for (let i = 0; i < clientes.length; i++) {
    ...
    try {
      ...
      console.error(
        `\n❌ Erro ao processar cliente ${cliente.id}:`,
        error.message
      );
      failed++;
    }
...
async function geocodeConcorrentes(limit = null) {
  ...
  for (let i = 0; i < concorrentes.length; i++) {
    ...
    try {
      ...
      console.error(
        `\n❌ Erro ao processar concorrente ${concorrente.id}:`,
        error.message
      );
      failed++;
    }
...
async function main() {
  ...
  try {
    ...
    if (results.clientes) {
      console.log(
        `Clientes: ${results.clientes.success}/${results.clientes.total} ✅`
      );
    ...
    if (results.concorrentes) {
      console.log(
        `Concorrentes: ${results.concorrentes.success}/${results.concorrentes.total} ✅`
      );

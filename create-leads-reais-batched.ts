...
async function createLeadsReaisProjectBatched() {
  console.log(
    "🚀 Criando projeto 'Leads Reais' com processamento em lotes...\n"
  );
  ...
  console.log(
    `📦 Dividindo em ${totalBatches} lotes de ${BATCH_SIZE} clientes\n`
  );
  ...
  for (let i = 0; i < totalBatches; i++) {
    ...
    console.log(`\n${"=".repeat(60)}`);
    console.log(
      `📦 Processando lote ${i + 1}/${totalBatches} (${batch.length} clientes)`
    );
    ...
    try {
      ...
      if (result.status === "completed" && result.data) {
        ...
        console.log(`\n✅ Lote ${i + 1} concluído!`);
        console.log(
          `   Clientes processados: ${totalProcessed}/${allClientes.length}`
        );

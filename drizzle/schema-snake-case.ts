...
export const alertConfigs = pgTable("alert_configs", {
  ...
  alert_type: varchar({ length: 50 }).notNull(),
...
export const alertHistory = pgTable("alert_history", {
  id: serial(),
  alert_config_id: integer().notNull(),
  project_id: integer().notNull(),
  alert_type: varchar({ length: 50 }).notNull(),
  condition: text().notNull(),
  message: text().notNull(),
  triggered_at: timestamp({ mode: "string" })
    .defaultNow()
    .notNull(),
});
...
export const analyticsDimensoes = pgTable("analytics_dimensoes", {
  ...
  project_id: integer().notNull(),
  pesquisa_id: integer(),
  dimensao_tipo: varchar({ length: 50 }).notNull(),
  dimensao_valor: varchar({ length: 100 }).notNull(),
  total_leads: integer().default(0),
  qualidade_media: integer().default(0),
  taxa_conversao_sf: integer().default(0),
  custo_medio_lead: integer().default(0),
...
export const analyticsMercados = pgTable("analytics_mercados", {
  ...
  total_concorrentes: integer().default(0),
  total_leads_gerados: integer().default(0),
  taxa_cobertura_mercado: integer().default(0),
  qualidade_media_leads: integer().default(0),
  leads_alta_qualidade: integer().default(0),
  leads_media_qualidade: integer().default(0),
  leads_baixa_qualidade: integer().default(0),
...
export const concorrentesHistory = pgTable("concorrentes_history", {
  id: serial(),
  concorrente_id: integer().notNull(),
  field: varchar({ length: 100 }),
  oldValue: text(),
  newValue: text(),
  changeType: varchar({ length: 50 }).default("updated"),
...
export const enrichmentJobs = pgTable("enrichment_jobs", {
  ...
  status: varchar({ length: 50 })
    .default("pending")
    .notNull(),
  total_clientes: integer().notNull(),
...
export const exportHistory = pgTable(
  "export_history",
  {
    ...
    pesquisa_id: integer().references(() => pesquisas.id, { onDelete: "cascade" }),
    context: text(),
    filters: jsonb(),
...
export const notifications = pgTable("notifications", {
  id: serial(),
  user_id: varchar({ length: 64 }).references(() => users.id, {
    onDelete: "cascade",
  }),
  project_id: integer().references(() => projects.id, { onDelete: "cascade" }),
...
export const users = pgTable("users", {
  ...
  senha_hash: varchar({ length: 255 }).notNull(),
  role: varchar({ length: 50 }).default("visualizador").notNull(),
  ativo: smallint().default(0).notNull(),
  liberado_por: varchar({ length: 64 }),
  liberado_em: timestamp({ mode: "string" }),
  created_at: timestamp({ mode: "string" }).defaultNow(),
  last_signed_in: timestamp({ mode: "string" }),
}, table => [
...
export const researchDrafts = pgTable("research_drafts", {
  ...
  user_id: varchar("userId", { length: 64 }).notNull(),

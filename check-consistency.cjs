...
async function main() {
  ...
  console.log('\n╔════════════════════════════════════════════════════════════════╗');
  console.log('║         VERIFICAÇÃO DE CONSISTÊNCIA E INTEGRIDADE             ║');
  console.log('╚════════════════════════════════════════════════════════════════╝\n');
  ...
  `);
  console.log(`   Produtos órfãos: ${orphanProducts[0].count} ${orphanProducts[0].count === 0 ? '✅' : '❌'}`);
  ...
  `);
  console.log(`   Concorrentes órfãos: ${orphanCompetitors[0].count} ${orphanCompetitors[0].count === 0 ? '✅' : '❌'}`);
  ...
  `);
  console.log(`   Leads órfãos: ${orphanLeads[0].count} ${orphanLeads[0].count === 0 ? '✅' : '❌'}\n`);
  
  const integrityOk = orphanProducts[0].count === 0 && 
  ...
  const [dupMercados] = await connection.query(`
    ...
    HAVING count > 1
  `);
  console.log(`   Mercados duplicados: ${dupMercados.length} ${dupMercados.length === 0 ? '✅' : '⚠️'}`);
  ...
  const [dupConcorrentes] = await connection.query(`
    ...
    HAVING count > 1
  `);
  console.log(`   Concorrentes duplicados: ${dupConcorrentes.length} ${dupConcorrentes.length === 0 ? '✅' : '⚠️'}`);
  ...
  const [dupLeads] = await connection.query(`
    ...
    HAVING count > 1
  `);
  console.log(`   Leads duplicados: ${dupLeads.length} ${dupLeads.length === 0 ? '✅' : '⚠️'}\n`);
  
  checks.push({
    name: 'Verificação de duplicatas',
    status: dupMercados.length === 0 && dupConcorrentes.length === 0 && dupLeads.length === 0 ? 'OK' : 'AVISO',
    details: {
  ...
  `);
  console.log(`   Clientes sem CNPJ: ${nullCNPJ[0].count} ${nullCNPJ[0].count === 0 ? '✅' : '⚠️'}`);
  ...
  `);
  console.log(`   Mercados sem nome: ${nullMercadoNome[0].count} ${nullMercadoNome[0].count === 0 ? '✅' : '❌'}`);
  ...
  const [nullProdutoNome] = await connection.query(`
    ...
    WHERE projectId = 1 AND (nome IS NULL OR nome = '')
  `);
  console.log(`   Produtos sem nome: ${nullProdutoNome[0].count} ${nullProdutoNome[0].count === 0 ? '✅' : '❌'}\n`);
  
  const fieldsOk = nullMercadoNome[0].count === 0 && nullProdutoNome[0].count === 0;
  ...
  const leadsGoodPercent = leadsQuality[0].total > 0
    ? ((leadsQuality[0].good / leadsQuality[0].total) * 100).toFixed(1)
    : 0;
  
  console.log(`   Leads:`);
  console.log(`     - Média: ${leadsAvg.toFixed(1)}`);
  console.log(`     - Range: ${leadsQuality[0].min || 0} - ${leadsQuality[0].max || 0}`);
  ...
  console.log(`   Clientes com Mercado: ${stats.clientesComMercado} (${((stats.clientesComMercado / stats.clientes) * 100).toFixed(1)}%)\n`);
  ...
  // Resumo final
  console.log('╔════════════════════════════════════════════════════════════════╗');
  console.log('║                    RESUMO DA CONSISTÊNCIA                      ║');
  console.log('╚════════════════════════════════════════════════════════════════╝\n');
  ...
  const overallStatus = failCount === 0 ? 
    (warnCount === 0 ? '✅ BANCO CONSISTENTE' : '⚠️  BANCO CONSISTENTE COM AVISOS') :
    '❌ BANCO COM PROBLEMAS';
  ...
  const consistencyResult = {
    ...
    summary: {
      ...
      failures: failCount,
      warnings: warnCount,
      overallStatus: failCount === 0 ? (warnCount === 0 ? 'CONSISTENT' : 'CONSISTENT_WITH_WARNINGS') : 'INCONSISTENT'
    }
  };
  
  const fs = require('fs');
  fs.writeFileSync('/home/ubuntu/CONSISTENCY_RESULTS.json', JSON.stringify(consistencyResult, null, 2));

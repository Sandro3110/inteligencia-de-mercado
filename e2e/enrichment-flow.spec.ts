...
test.describe('Fluxo de Enriquecimento', () => {
  ...
  test('deve navegar para página de enriquecimento', async ({ page }) => {
    ...
    // Verificar título
    await expect(page.locator('h1, h2').first()).toContainText(/Enriquecimento|Configurações/i);
  });
  ...
  test('deve validar campos obrigatórios', async ({ page }) => {
    ...
    if (await saveButton.isVisible()) {
      ...
      const errorVisible = await page.locator('text=/obrigatório|required|inválido|invalid/i').isVisible().catch(() => false);
  ...
  test('deve exibir progresso de enriquecimento', async ({ page }) => {
    ...
    const hasProgress = await page.locator('text=/Progresso|Progress|Status/i').isVisible();
  ...
  test('deve validar API Key com Zod', async ({ page }) => {
    ...
    if (await apiKeyInput.isVisible()) {
      ...
      if (await testButton.isVisible()) {
        ...
        const errorVisible = await page.locator('text=/pelo menos 10 caracteres|inválido/i').isVisible().catch(() => false);
        expect(errorVisible).toBeTruthy();
      }
    }
  });

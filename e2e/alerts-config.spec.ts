...
test.describe("Configuração de Alertas", () => {
  ...
  test("deve exibir configurações de alertas", async ({ page }) => {
    ...
    const hasThreshold = await page
      .locator("text=/Threshold|Limite/i")
      .isVisible();
    ...
    if (await thresholdInput.isVisible()) {
      ...
      if (await saveButton.isVisible()) {
        ...
        const errorVisible = await page
          .locator("text=/máximo|inválido|invalid/i")
          .isVisible()
          .catch(() => false);
    ...
    if (count > 0) {
      ...
      if (await saveButton.isVisible()) {
        ...
        const successVisible = await page
          .locator("text=/sucesso|success|salvo/i")
          .isVisible()
          .catch(() => false);
  ...
  test("deve exibir histórico de alertas", async ({ page }) => {
    ...
    const hasTitle = await page
      .locator("text=/Histórico|History/i")
      .isVisible();
  ...
  test("deve exibir estatísticas de alertas", async ({ page }) => {
    ...
    const hasStats = await page
      .locator("text=/Estatísticas|Stats|24h|Total/i")
      .isVisible();

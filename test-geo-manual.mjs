...
try {
  ...
  // Cliente
  console.log('📍 CLIENTE:');
  console.log(`  Nome: ${result.clienteEnriquecido?.produtoPrincipal || 'N/A'}`);
  console.log(`  Cidade: ${result.clienteEnriquecido?.cidade || 'N/A'}`);
  console.log(`  UF: ${result.clienteEnriquecido?.uf || 'N/A'}`);
  console.log(`  Latitude: ${result.clienteEnriquecido?.latitude ?? 'NÃO RETORNADO'}`);
  console.log(`  Longitude: ${result.clienteEnriquecido?.longitude ?? 'NÃO RETORNADO'}`);
  ...
  for (const c of concorrentes.slice(0, 3)) {
    ...
    console.log(`    Lat/Lng: ${hasGeo ? `${c.latitude}, ${c.longitude}` : 'NÃO RETORNADO'}`);
  }
  ...
  for (const l of leads.slice(0, 3)) {
    ...
    console.log(`    Lat/Lng: ${hasGeo ? `${l.latitude}, ${l.longitude}` : 'NÃO RETORNADO'}`);
  }
  ...
  console.log(`  Cliente: ${clienteHasGeo ? '✅ COM coordenadas' : '❌ SEM coordenadas'}`);
  console.log(`  Concorrentes: ${concorrentesComGeo}/${concorrentes.length} com coordenadas`);

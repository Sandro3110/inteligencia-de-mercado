...
async function auditOrphanRecords() {
  ...
  if (pesquisasCount > 0) {
    results.push({
      ...
      query: 'SELECT * FROM pesquisas WHERE projectId NOT IN (SELECT id FROM projects)',
  ...
  if (mercadosCount > 0) {
    results.push({
      ...
      count: mercadosCount,
      query: 'SELECT * FROM mercados_unicos WHERE pesquisaId IS NULL OR pesquisaId NOT IN (SELECT id FROM pesquisas)',
  ...
  if (clientesCount > 0) {
    results.push({
      ...
      count: clientesCount,
      query: 'SELECT * FROM clientes WHERE pesquisaId IS NULL OR pesquisaId NOT IN (SELECT id FROM pesquisas)',
  ...
  if (concorrentesCount > 0) {
    results.push({
      ...
      query: 'SELECT * FROM concorrentes WHERE pesquisaId IS NULL OR pesquisaId NOT IN (SELECT id FROM pesquisas)',
  ...
  if (leadsCount > 0) {
    results.push({
      ...
      count: leadsCount,
      query: 'SELECT * FROM leads WHERE pesquisaId IS NULL OR pesquisaId NOT IN (SELECT id FROM pesquisas)',
  ...
  if (produtosCount > 0) {
    results.push({
      ...
      count: produtosCount,
      query: 'SELECT * FROM produtos WHERE clienteId NOT IN (SELECT id FROM clientes)',
...
async function auditDuplicates() {
  ...
  if (dupClientesCount > 0) {
    results.push({
      ...
      query: 'SELECT clienteHash, COUNT(*) as count FROM clientes GROUP BY clienteHash HAVING count > 1',
  ...
  if (dupConcorrentesCount > 0) {
    results.push({
      ...
      query: 'SELECT concorrenteHash, COUNT(*) as count FROM concorrentes GROUP BY concorrenteHash HAVING count > 1',
  ...
  if (dupLeadsCount > 0) {
    results.push({
      ...
      count: dupLeadsCount,
      query: 'SELECT leadHash, COUNT(*) as count FROM leads GROUP BY leadHash HAVING count > 1',
  ...
  if (dupMercadosCount > 0) {
    results.push({
      ...
      query: 'SELECT nome, COUNT(*) as count FROM mercados_unicos GROUP BY nome HAVING count > 1',
      suggestion: 'Mercados podem ter nomes iguais em pesquisas diferentes (OK)'
    });
  }
}
...
async function auditConsistency() {
  ...
  if (inconsistentCount > 0) {
    results.push({
      ...
      query: 'SELECT p.id, p.nome, p.totalClientes as declarado, COUNT(c.id) as real FROM pesquisas p LEFT JOIN clientes c ON c.pesquisaId = p.id GROUP BY p.id HAVING declarado != real',
...
`;
  ...
  fs.writeFileSync('/home/ubuntu/gestor-pav/RELATORIO_AUDITORIA_BANCO.md', report);
...
async function main() {
  ...
  try {
    ...
    if (results.length === 0) {
      ...
      console.log('📄 Veja o relatório completo em: RELATORIO_AUDITORIA_BANCO.md\n');
    }

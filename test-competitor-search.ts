  ...
  try {
    ...
    const validations = [
      ...
      },
      {
        ...
        passed: competitorData.concorrentes.every((c: any) => c.nome && c.nome.length > 0),
      },
      {
        test: 'Todos os concorrentes têm produto',
        passed: competitorData.concorrentes.every((c: any) => c.produto && c.produto.length > 0),
      },
      {
        test: 'Concorrentes relacionados ao setor automotivo',
        passed: competitorData.concorrentes.some((c: any) => 
    ...
    console.log('\n' + '═'.repeat(70));
    if (allPassed) {
      console.log('✅ TESTE APROVADO - Busca de concorrentes funcionando perfeitamente');
    } else if (criticalPassed) {
      console.log('⚠️  TESTE PARCIAL - Funcionalidade core OK, enriquecimento pode melhorar');
    } else {
    ...
    // Estatísticas
    console.log('\n📊 ESTATÍSTICAS:');
    console.log(`  Total de concorrentes: ${competitorData.concorrentes.length}`);
    console.log(`  Enriquecidos com sucesso: ${enrichedCompetitors.filter((c: any) => c.enriched).length}`);
    console.log(`  Taxa de enriquecimento: ${((enrichedCompetitors.filter((c: any) => c.enriched).length / competitorData.concorrentes.length) * 100).toFixed(1)}%`);

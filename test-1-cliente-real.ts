...
import { enrichClienteReal } from './server/enrichmentReal';
...
async function main() {
  ...
  try {
    ...
    if (result.success) {
      ...
      // Mercados
      const [mercados] = await connection.query<any[]>(`
      ...
      console.log(`\n🏢 Concorrentes (Top 5):`);
      concorrentes.forEach((c: any, i: number) => {
      ...
      // Leads
      const [leads] = await connection.query<any[]>(`
      ...
      const avgConcorrentes = concorrentes.reduce((sum: number, c: any) => sum + c.qualidadeScore, 0) / concorrentes.length;
      const avgLeads = leads.reduce((sum: number, l: any) => sum + l.qualidadeScore, 0) / leads.length;

...
interface SectorsViewProps {
  ...
  pesquisaIds: number[];
  onBack: () => void;
  onDrillDown: (setorNome: string, tipo: 'clientes' | 'leads' | 'concorrentes') => void;
}

/**
 * NÍVEL 2: Visualização de setores de uma categoria
 * Mostra lista de setores com contadores e botões para drill-down
 */
export function SectorsView({ categoria, pesquisaIds, onBack, onDrillDown }: SectorsViewProps) {
  const [currentPage, setCurrentPage] = useState(1);
  const pageSize = 50;

  const { data, isLoading } = trpc.sectorDrillDown.getSectors.useQuery({
    categoria,
    pesquisaIds,
    limit: pageSize,
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            {total} setor{total !== 1 ? 'es' : ''} encontrado{total !== 1 ? 's' : ''}
          </p>
        </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Clique nos botões "Ver" para explorar clientes, leads ou concorrentes de cada setor
          </CardDescription>
        </CardHeader>
        <CardContent>
          <DrillDownTable
            ...
            emptyMessage="Nenhum setor encontrado nesta categoria"
          />
        </CardContent>
      </Card>
    </div>

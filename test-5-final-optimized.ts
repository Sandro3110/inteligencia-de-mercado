...
async function main() {
  ...
  console.log('\n╔════════════════════════════════════════════════════════════════╗');
  console.log('║  TESTE FINAL: Prompt Estruturado de Alta Qualidade (5 clientes) ║');
  console.log('╚════════════════════════════════════════════════════════════════╝\n');
  ...
  try {
    ...
    console.log(`\n⏱️  Performance:`);
    console.log(`   Tempo Total: ${totalDuration}s (${(parseFloat(totalDuration)/60).toFixed(1)} min)`);
    ...
    console.log(`   - Concorrentes: ${stat.avgScoreConcorrentes ? stat.avgScoreConcorrentes.toFixed(1) : 'N/A'}/100`);
    console.log(`   - Leads: ${stat.avgScoreLeads ? stat.avgScoreLeads.toFixed(1) : 'N/A'}/100`);
    console.log(`\n🎯 Dados Enriquecidos (Porte, Região, Justificativas):`);
    console.log(`   - Concorrentes com Porte: ${stat.concorrentesComPorte}/${stat.totalConcorrentesRecentes} (${stat.totalConcorrentesRecentes > 0 ? (stat.concorrentesComPorte/stat.totalConcorrentesRecentes*100).toFixed(0) : 0}%)`);
    console.log(`   - Leads com Porte: ${stat.leadsComPorte}/${stat.totalLeadsRecentes} (${stat.totalLeadsRecentes > 0 ? (stat.leadsComPorte/stat.totalLeadsRecentes*100).toFixed(0) : 0}%)`);
    console.log(`   - Leads com Justificativa: ${stat.leadsComJustificativa}/${stat.totalLeadsRecentes} (${stat.totalLeadsRecentes > 0 ? (stat.leadsComJustificativa/stat.totalLeadsRecentes*100).toFixed(0) : 0}%)`);
    ...
    exemploConcorrentes.forEach((c: any, i: number) => {
      console.log(`   ${i+1}. ${c.nome} | Porte: ${c.porte || 'N/A'} | Região: ${c.regiao || 'N/A'} | Score: ${c.qualidadeScore}`);
    });
    
    console.log(`\n💼 Top 5 Leads (por quality score):`);
    exemploLeads.forEach((l: any, i: number) => {
      console.log(`   ${i+1}. ${l.nome} | Porte: ${l.porte || 'N/A'} | Potencial: ${l.tipo} | Score: ${l.qualidadeScore}`);
      if (l.observacoes) {
        console.log(`      💡 ${l.observacoes.substring(0, 80)}${l.observacoes.length > 80 ? '...' : ''}`);
      }
    });
    ...
    console.log(`   💰 Custo Estimado OpenAI: $${projectedCost.toFixed(2)} USD`);
    console.log(`   📊 Registros Esperados: ~${Math.round((totalMercados + totalProdutos + totalConcorrentes + totalLeads) * 801 / 5)} registros`);
    ...
    const report = {
      ...
      clientes: clientes.map((c: any) => ({ id: c.id, nome: c.nome, cidade: c.cidade })),
      results,
      summary: {
        successCount,
        failCount,
        ...
        enrichmentRate: {
          concorrentesComPorte: stat.totalConcorrentesRecentes > 0 ? stat.concorrentesComPorte/stat.totalConcorrentesRecentes : 0,
          leadsComPorte: stat.totalLeadsRecentes > 0 ? stat.leadsComPorte/stat.totalLeadsRecentes : 0,
          leadsComJustificativa: stat.totalLeadsRecentes > 0 ? stat.leadsComJustificativa/stat.totalLeadsRecentes : 0
        }
      },
      examples: {
      ...
      projection: {
        time: projectedTime,
        cost: projectedCost,
        records: Math.round((totalMercados + totalProdutos + totalConcorrentes + totalLeads) * 801 / 5)
      }
    };
    
    fs.writeFileSync('/home/ubuntu/TEST_FINAL_OPTIMIZED_REPORT.json', JSON.stringify(report, null, 2));

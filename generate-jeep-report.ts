...
async function generateReport() {
  ...
  const projeto = await db.select().from(projects).where(eq(projects.nome, 'Teste Jeep do Brasil')).limit(1);
  ...
  // Buscar mercado
  const mercado = await db.select().from(mercadosUnicos).where(eq(mercadosUnicos.projectId, projectId));
  ...
  // Buscar cliente
  const cliente = await db.select().from(clientes).where(eq(clientes.projectId, projectId));
  console.log(`\n--- CLIENTE PROCESSADO ---`);
  if (cliente.length > 0) {
    const c = cliente[0];
  ...
  // Buscar concorrentes
  const concorrentesList = await db.select().from(concorrentes).where(eq(concorrentes.projectId, projectId));
  ...
  // Buscar leads
  const leadsList = await db.select().from(leads).where(eq(leads.projectId, projectId));

...
function analyzeEntities(entities: any[], type: string): AnalysisResult {
  const invalidPatterns: Map<string, { count: number; examples: Set<string> }> =
    new Map();
  ...
  entities.forEach(entity => {
    ...
    blockedDomains.forEach(domain => {
      if (site.includes(domain)) {
        addPattern(
          invalidPatterns,
          `Site de domínio bloqueado: ${domain}`,
          nome
        );
      }
    });
    ...
    genericNames.forEach(generic => {
      if (
        nome.toLowerCase() === generic ||
        nome.toLowerCase().startsWith(generic + " ")
      ) {
    ...
    const hasCorporateDomain = site.match(
      /\.(com\.br|ind\.br|net\.br|com|net)$/
    );
  ...
  const patternsArray = Array.from(invalidPatterns.entries()).map(
    ([pattern, data]) => ({
      pattern,
      count: data.count,
  ...
  console.log(
    `Registros com problemas: ${patternsArray.reduce((sum, p) => sum + p.count, 0)}`
  );
...
function generateFilterRecommendations(patterns: any[]) {
  ...
  if (nameKeywords.length > 0) {
    console.log(
      "1. PALAVRAS-CHAVE DE NOMES (adicionar a ARTICLE_TITLE_KEYWORDS):"
    );
  ...
  if (longNames && longNames.count > 10) {
    console.log(
      `   - Rejeitar nomes com mais de 80 caracteres (${longNames.count} casos)`
    );
  }

  const punctuation = patterns.find(p =>
    p.pattern.includes("pontuação excessiva")
  );
  if (punctuation && punctuation.count > 10) {
    console.log(
      `   - Rejeitar nomes com pontuação excessiva (${punctuation.count} casos)`
    );
  }

  const startsWithNumber = patterns.find(p =>
    p.pattern.includes("inicia com número")
  );
  if (startsWithNumber && startsWithNumber.count > 10) {
    console.log(
      `   - Rejeitar nomes que iniciam com número (${startsWithNumber.count} casos)`
    );
  }
...
function generateReport(data: any): string {
  ...
  report +=
    "Com base nos padrões identificados, recomendo atualizar os filtros:\n\n";

...
async function reprocessInBatches() {
  ...
  try {
    ...
    if (finalResult) {
      ...
      console.log(
        `  Score médio: ${finalResult.stats?.avgQualityScore?.toFixed(1) || "N/A"}/100\n`
      );
      ...
      console.log(`📁 Projeto ID: ${finalResult.projectId}`);
      console.log(
        `📊 Acesse a aplicação e filtre por "Embalagens 2" para ver os resultados\n`
      );
      ...
      console.log(
        "  Para processar todos, execute o script completo (pode levar 30-60 min)\n"
      );
    }
  } catch (error) {

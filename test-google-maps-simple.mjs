...
async function testGeocodingWithApiKey() {
  ...
  if (API_KEY === "SUA_API_KEY_AQUI") {
    ...
    console.log(
      "1. Acesse: https://console.cloud.google.com/apis/credentials?project=metal-incline-451301-r0"
    );
    console.log('2. Clique em "Create Credentials" > "API key"');
    console.log("3. Copie a chave gerada");
    console.log(
      "4. Habilite a Geocoding API em: https://console.cloud.google.com/apis/library/geocoding-backend.googleapis.com?project=metal-incline-451301-r0"
    );
  ...
  try {
    ...
    if (data.status === "OK" && data.results.length > 0) {
      ...
      console.log(
        `   - Mensagem: ${data.error_message || "Nenhum resultado"}\n`
      );

      if (data.status === "REQUEST_DENIED") {
        console.log(
          "💡 Dica: Verifique se a Geocoding API está habilitada no projeto"
        );
        console.log(
          "   Link: https://console.cloud.google.com/apis/library/geocoding-backend.googleapis.com?project=metal-incline-451301-r0\n"
        );
      }
